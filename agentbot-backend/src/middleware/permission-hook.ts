...
export async function preToolUseHook(
  ...
  try {
    ...
    log.error('[PermissionHook] Failed to persist pending request:', { error: message })
...
export async function getPendingForUser(userId: string): Promise<PendingRequest[]> {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    log.error('[PermissionHook] getPendingForUser failed:', { error: message })
    return []
  }
}
...
export async function getPendingForAgent(agentId: string): Promise<PendingRequest[]> {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    log.error('[PermissionHook] getPendingForAgent failed:', { error: message })
    return []
  }
}
...
export async function processPermissionDecision(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    log.error('[PermissionHook] processPermissionDecision failed:', { error: message })
    return null
  }
}
