...
export async function POST(req: NextRequest) {
  ...
  const { signatureType } = body as { signatureType?: string }
  if (!signatureType) {
    return NextResponse.json({ error: 'signatureType is required' }, { status: 400 })
  }

  try {
    const res = await fetch(`${BANKR_API_URL}/wallet/sign`, {
      method: 'POST',
