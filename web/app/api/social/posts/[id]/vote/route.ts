...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.socialVote.findFirst({
      where: { postId: id, userId: localUser.id },
    });
    ...
    if (existing) {
      ...
      await prisma.$transaction([
        ...
        prisma.post.update({ where: { id }, data: { voteCount: { increment: diff } } }),
      ]);
    } else {
      await prisma.$transaction([
        prisma.socialVote.create({ data: { postId: id, userId: localUser.id, value } }),
        prisma.post.update({ where: { id }, data: { voteCount: { increment: value } } }),
      ]);
    }

    const updated = await prisma.post.findUnique({ where: { id }, select: { voteCount: true } });
