  ...
  try {
    ...
    if (!sessionId || !Array.isArray(files) || files.length === 0) {
      return Response.json(
        { ok: false, error: 'sessionId and files[] are required' },
        { status: 400 }
      );
    }
    ...
    if (runInstall && hasPackageJson) {
      ...
      while (retries > 0) {
        try {
          ...
          if (installResult?.command?.exitCode === 0) break;

          retries--;
          if (retries > 0) {
            console.log('[sandbox.files] npm install failed, retrying...');
      ...
      if (installResult?.command?.exitCode !== 0) {
        return Response.json({
          ok: true,
          installError: installResult?.command?.stderr || 'npm install failed after retries',
        });
      }
    }
