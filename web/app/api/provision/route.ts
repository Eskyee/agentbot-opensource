  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (enqueueRes.status < 500 && enqueueRes.status !== 408 && enqueueRes.status !== 429)
          break;
        lastError = `HTTP ${enqueueRes.status}`;
      } catch (err) {
        lastError = err instanceof Error ? err.message : String(err);
        const transient = /AbortError|timeout|ETIMEDOUT|ECONNRESET|ENOTFOUND|fetch failed/i.test(
          lastError
        );
    ...
    if (contentType.includes('application/json')) {
      ...
      console.error(
        '[Provision] Backend returned non-JSON:',
        enqueueRes.status,
        rawText.slice(0, 300)
      );
      data = {
        error: `Backend unavailable (HTTP ${enqueueRes.status}). Please try again in a moment.`,
      };
    }
