...
const BUDDY_TYPES: Record<BuddyType, { emoji: string; name: string; rarity: string; color: string; border: string; desc: string }> = {
  ...
  ghost: { emoji: '👻', name: 'Ghost Agent', rarity: 'Rare', color: 'from-orange-500 to-amber-500', border: 'border-orange-500/30', desc: 'Mysterious and elusive. Hard to find in eggs.' },
    ...
    return (
      <main className="min-h-screen bg-black text-white font-mono flex items-center justify-center">
        <div className="text-center">
          <Egg className="w-12 h-12 mx-auto mb-3 animate-bounce text-orange-400" />
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl max-w-md w-full p-6 relative">
            ...
            <div className="flex gap-3">
              ...
              {tutorialStep < TUTORIAL_STEPS.length - 1 ? (
                <button onClick={() => setTutorialStep(tutorialStep + 1)}
                  className="flex-1 bg-gradient-to-r from-orange-500 to-amber-500 py-3 rounded-lg font-bold text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
      ...
      {hatchResult && !hatchAnimation && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setHatchResult(null)}>
          <div className="text-center animate-bounce" onClick={e => e.stopPropagation()}>
            <div className={`w-32 h-32 rounded-full bg-gradient-to-br ${BUDDY_TYPES[hatchResult.type].color} flex items-center justify-center text-7xl mx-auto mb-6 shadow-2xl shadow-orange-500/20`}>
            ...
            <button onClick={() => setHatchResult(null)}
              className="bg-gradient-to-r from-orange-500 to-amber-500 px-8 py-3 rounded-xl font-bold hover:opacity-90 transition-opacity">
              Awesome!
            </button>
          </div>
        </div>
        ...
        {buddies.length === 0 && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-8">
            <h2 className="text-lg font-bold uppercase tracking-tight mb-4 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-orange-400" /> Getting Started
            </h2>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={hatchNewBuddy} disabled={loading}
                className="bg-gradient-to-r from-orange-500 to-amber-500 px-4 py-2 rounded-lg font-bold text-sm hover:opacity-90 transition-opacity disabled:opacity-50">
                {loading ? (
            ...
            <div className={`bg-zinc-900 border rounded-2xl p-6 mb-8 ${info.border} border-zinc-800`}>
                    ...
                    {renamingId === selectedBuddy.id ? (
                      <div className="flex items-center gap-2">
                        <input value={renameValue} onChange={e => setRenameValue(e.target.value)}
                          ...
                          className="bg-zinc-800 border border-zinc-600 rounded px-2 py-1 text-lg font-bold w-32 focus:outline-none focus:border-orange-500"
              ...
              <div className="grid gap-3 sm:grid-cols-3 mb-6">
                ...
                <div className="bg-zinc-800 rounded-lg p-3">
                  ...
                  <div className="w-full bg-zinc-700 rounded-full h-2.5">
                    <div className="bg-orange-400 h-2.5 rounded-full transition-all duration-500" style={{ width: `${selectedBuddy.xp % 100}%` }} />
                  </div>
