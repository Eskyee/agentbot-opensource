...
const TEMPLATES: Template[] = [
  {
    ...
    description:
      'Autonomous research dashboard. Web scraping, competitor monitoring, briefing reports.',
    category: 'agent',
    icon: Terminal,
    prompt:
      'Build a research agent dashboard in src/App.tsx. Header with agent status (Running/Idle). Three panels: a search input with results list, a briefing generator that shows structured reports, and a monitoring feed with live updates. Include sample research data. Dark theme with green status accents.',
  ...
  },
  {
    ...
    icon: Globe2,
    prompt:
      'Build a social media promoter dashboard in src/App.tsx. Content calendar grid showing scheduled posts, a post composer with platform toggles (Twitter/Instagram/LinkedIn), engagement metrics cards (likes, shares, comments), and a draft queue. Dark theme with platform-specific accent colors.',
  ...
  },
  {
    ...
    icon: FileText,
    prompt:
      'Build a bookings and finance agent dashboard in src/App.tsx. Booking calendar with upcoming events, an invoice generator with line items and totals, a payment history table with status badges (Paid/Pending/Overdue), and a wallet balance card showing USDC on Base. Dark theme with orange accents.',
  ...
  },
  {
    ...
    icon: Music,
    prompt:
      'Build a live DJ dashboard in src/App.tsx. Now playing card with track info and waveform, queue of upcoming tracks, listener count, a BPM mixer panel with tempo controls, and a chat feed. Dark theme with neon purple/green accents. Include sample track data.',
  ...
  },
  {
    ...
    icon: Monitor,
    prompt:
      'Build a data analyst dashboard in src/App.tsx. KPI cards with sparklines, a bar chart for daily metrics, a line chart for trend analysis, a data table with sorting, and an export button. Seed with 30 days of sample data. Dark theme with blue accent colors.',
  ...
  },
  {
    ...
    icon: Layout,
    prompt:
      'Build a landing page in src/App.tsx for an AI infrastructure startup. Hero with gradient backdrop, three feature cards, a pricing table with three tiers (Free / Pro / Enterprise), a testimonials row, and a footer. Dark theme with subtle gradients in src/index.css. Keep it semantic and accessible.',
  ...
  },
  {
    ...
    icon: Layout,
    prompt:
      'Build a SaaS pricing page in src/App.tsx with three tiers. Feature comparison grid, toggle for monthly/annual with discount, FAQ accordion with 4 questions, and a prominent CTA button. Clean dark theme with accent color on the popular tier.',
  ...
  },
  {
    ...
    icon: Monitor,
    prompt:
      'Build a crypto portfolio tracker in src/App.tsx. Header with total balance and 24h change. Grid of token cards (ETH, USDC, BTC, SOL) with price and holding value. A simple allocation bar chart. Transaction history list with type (buy/sell), amount, and timestamp. Dark theme.',
  ...
  },
  {
    ...
    icon: FileText,
    prompt:
      'Build an invoice generator in src/App.tsx. Form with client name/email, dynamic line items (description, quantity, rate), auto-calculated subtotal/VAT/total. A live invoice preview panel on the right with professional formatting. Add/remove line items dynamically. Dark theme.',
  ...
  },
  {
    ...
    icon: Send,
    prompt:
      'Build a chat interface in src/App.tsx. Message list with sent/received bubbles (different colors), timestamps, a typing indicator animation, and a message input with send button. Seed with 8 sample messages. Use React state for sending new messages that appear instantly.',
  ...
  },
  {
    ...
    icon: Globe2,
    prompt:
      'Build a portfolio site in src/App.tsx for a product designer. Hero, three case-study cards with hover states, a selected-work gallery (CSS grid), and a contact form that validates email format and shows inline errors. Sans-serif typography, warm neutrals.',
...
const PLAYGROUND_PROMISE =
  'Build and publish Vite + React apps with OpenClaude, Xiaomi MiMo, and Agentbot gateway routing.';
...
export default function PlaygroundPage() {
    ...
    void fetch(`/api/playground/share/${encodeURIComponent(remixId)}`)
      ...
      .then((data) => {
        ...
        const project: PlaygroundProject = {
          id: createId(),
          name: `${(data.name || data.title || 'remix')
            .toString()
            .replace(/-remix$/, '')}-remix`.slice(0, 64),
          status: 'IDLE',
          template: 'VITE-REACT-TS',
          lastActive: 'now',
          generation: {
  ...
  const breadcrumb =
    view === 'builder' ? activeProject?.name || generation?.title || 'untitled' : view;
        ...
        if (proj?.generation) {
          ...
          if (source?.generation) {
            const newProj: PlaygroundProject = {
              ...
              generation: {
                ...source.generation,
                files: source.generation.files.map((f) => ({ ...f })),
              },
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          body?.error ||
          (res.status === 409
            ? 'Generate the app before creating an agent'
            : res.status === 404
  ...
  function remixProject(projectId: string) {
    ...
    const project: PlaygroundProject = {
      ...
      generation: {
        ...source.generation,
        files: source.generation.files.map((file) => ({ ...file })),
      },
      messages: source.messages ? [...source.messages] : undefined,
    };
    setProjects((current) => [project, ...current]);
  ...
  async function publishProject() {
    ...
    try {
      const response = await fetch(
        `/api/playground/projects/${encodeURIComponent(activeProject.id)}/publish`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (response.status === 401) {
        setPublishError(
          'Sign in to publish this project to Vercel. Your files are still saved locally in this browser.'
        );
        return;
      }
  ...
  async function pushProjectToGitlawb() {
    ...
    try {
      const response = await fetch(
        `/api/playground/projects/${encodeURIComponent(activeProject.id)}/gitlawb`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...activeProject, prompt, provider, model }),
      ...
      if (response.status === 401) {
        throw new Error(
          'Sign in to push this project to GitLawb. Your files are still saved locally in this browser.'
        );
      }
      if (!response.ok) {
      ...
      if (pushed) {
        setStorage('server');
        setProjects((current) =>
          current.map((project) => (project.id === pushed.id ? { ...project, ...pushed } : project))
        );
      }
    } catch (err) {
    ...
    updateActiveProject(
      {
        ...
        history: snapshotForHistory(
          activeProject,
          cleanPrompt || activeProject.generation?.title || 'edit'
        ),
      },
      { prompt: cleanPrompt, provider: data.provider, model: data.model }
    );
    setSelectedFile(
      data.generation.files.find((file) => file.path === 'src/App.tsx')?.path ??
        data.generation.files[0]?.path ??
        '.gitignore'
    );
    appendMessage({
      role: 'assistant',
      content: `${data.generation.title} — ${data.generation.summary}`,
      at: Date.now(),
    });
  }
  ...
  function restoreSnapshot(at: number) {
    ...
    updateActiveProject({
      ...
      history: snapshotForHistory(
        activeProject,
        `before restore · ${new Date(snap.at).toLocaleTimeString()}`
      ),
    });
    setSelectedFile('src/App.tsx');
  ...
  async function submitViaStream(cleanPrompt: string): Promise<boolean> {
    ...
    const upsert = (path: string, content: string) => {
      ...
      if (index >= 0) {
        working = {
          ...working,
          files: working.files.map((file, i) => (i === index ? { ...file, content } : file)),
        };
      } else {
    ...
    while (true) {
      ...
      for (const frame of frames) {
        ...
        if (event.type === 'meta') {
          working = {
            ...working,
            title: String(event.title || working.title),
            summary: String(event.summary || working.summary),
          };
        } else if (
          ...
          finishGeneration(
            {
              provider: String(event.provider || 'vercel-ai-gateway'),
              model: String(event.model || model),
              generation: event.generation as PlaygroundGeneration,
            },
            cleanPrompt
          );
  ...
  async function submit(nextPrompt = prompt) {
    ...
    if (!autoApprove) {
      const confirmed = window.confirm(
        `Generate app for: "${cleanPrompt.slice(0, 80)}${cleanPrompt.length > 80 ? '…' : ''}"?`
      );
      if (!confirmed) return;
    }
      ...
      <div className="border-b border-zinc-900 bg-black">
        <div className="min-h-12 px-4 py-2 flex flex-col gap-2 lg:h-12 lg:flex-row lg:items-center lg:justify-between lg:py-0">
          ...
          <div className="flex items-center gap-2 overflow-x-auto text-[10px] uppercase tracking-widest text-zinc-500 lg:overflow-visible">
            ...
            >
              {agentMade && agentLink ? (
                <a href={agentLink} className="underline">
                  ✓ Agent listed
                </a>
              ) : agentMade ? (
                '✓ Agent listed'
            ...
            {agentError && (
              <span className="text-[10px] text-red-400 max-w-[180px] truncate" title={agentError}>
                {agentError}
              </span>
            )}
            <button
              type="button"
              onClick={newProject}
              className="inline-flex items-center gap-1 border border-zinc-800 px-2 py-1 hover:text-white"
            >
              <Plus className="h-3 w-3" />
              New project
            </button>
            <a href="/vercel-gateway" className="hidden px-2 py-1 hover:text-white md:inline">
              Gateway
            </a>
            <a href="/logout" className="hidden px-2 py-1 hover:text-white sm:inline">
              Sign out
            </a>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md border border-zinc-800 bg-black p-8">
            <button
              ...
              className="absolute right-4 top-4 text-zinc-600 hover:text-white"
            >
              <X className="h-4 w-4" />
            </button>
            ...
            <p className="mt-3 text-sm leading-relaxed text-zinc-400">
              You&apos;ve used all 3 free generations today. Subscribe to generate unlimited apps,
              open full sandboxes, and deploy to production.
            </p>
      ...
      {showEmailCapture && !emailCaptureSent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md border border-zinc-800 bg-black p-8">
            ...
            <h2 className="mt-3 text-2xl font-bold uppercase tracking-tighter">
              Don&apos;t lose your app
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-zinc-400">
              Enter your email to save this project. We&apos;ll also send you tips on how to deploy
              it.
            </p>
            <form
              onSubmit={async (e) => {
      ...
      <div className="flex min-h-[calc(100vh-9rem)] items-center justify-center px-5 py-16">
        <div className="w-full max-w-2xl text-center">
          ...
          <h1 className="mt-4 text-4xl font-bold uppercase leading-[0.9] tracking-tighter sm:text-5xl">
            What do you want
            <br />
            to <span className="text-orange-500">create?</span>
          </h1>
            ...
            {messages.length > 0 && (
              <div className="max-h-72 space-y-2 overflow-y-auto border border-zinc-900 bg-zinc-950/50 p-3">
                {messages.map((message, index) => (
                  ...
                  >
                    <div
                      className={`inline-block max-w-[90%] border px-3 py-2 text-left text-xs leading-relaxed ${
                        message.role === 'user'
                    ...
                    >
                      <div
                        className={`mb-1 text-[9px] uppercase tracking-widest ${
                          message.role === 'user' ? 'text-zinc-500' : 'text-orange-500'
                        }`}
                      >
            ...
            <div className={messages.length > 0 ? 'hidden' : 'space-y-2'}>
              {TEMPLATES.slice(0, 6).map((template) => (
                ...
                >
                  ...
                  <div className="mt-2 line-clamp-3 text-xs leading-relaxed text-zinc-500">
        ...
        <div className="border-b border-zinc-900 px-4 py-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className={`ml-2 inline-flex h-8 items-center gap-2 border px-3 text-[10px] font-bold uppercase tracking-widest transition-colors ${
                showSandboxConsole
                  ? 'border-orange-500 text-orange-500'
                  : 'border-zinc-800 text-zinc-500 hover:text-white'
              }`}
          ...
          <div className="flex items-center justify-between gap-3 border-b border-red-900/70 bg-red-950/20 px-4 py-2 text-xs text-red-400">
            <span className="min-w-0 truncate" title={runtimeError.message}>
              ...
              {runtimeError.filePath
                ? `${runtimeError.filePath}${runtimeError.line ? `:${runtimeError.line}` : ''} — `
                : ''}
            ...
            {fixCount < 3 ? (
              <button
                ...
                onClick={() => {
                  setFixCount((c) => c + 1);
                  const loc = runtimeError.filePath
                    ? ` in ${runtimeError.filePath}${
                        runtimeError.line ? ` line ${runtimeError.line}` : ''
                      }`
                    : '';
                  void submit(
                    `Fix this error${loc} without changing anything else: ${runtimeError.message}`
                  );
                }}
        ...
        <div className="grid flex-1 min-h-[560px] xl:grid-cols-[1fr_280px]">
            ...
            {isGenerating && (!generation || generation.files.length === 0) ? (
              <div className="h-full min-h-[520px] border border-zinc-900 bg-black flex items-center justify-center">
                <div className="max-w-md px-6 text-center">
                  ...
                  <div
                    className="mt-4 text-[10px] uppercase tracking-widest text-orange-500"
                    aria-live="polite"
                  >
                    {spinnerVerb}…
                  </div>
              ...
              <div className="h-full min-h-[520px] overflow-auto border border-zinc-900 bg-zinc-950 p-4">
                {isGenerating && (
                  <div className="mb-3 flex items-center gap-2 border border-orange-500/30 bg-orange-500/5 px-3 py-2 text-[10px] uppercase tracking-widest text-orange-500">
                    <Spinner size={12} />
                    {spinnerVerb}…{' '}
                    {executionMode === 'sandbox' ? 'deploying to VM' : 'streaming to preview'}
                  </div>
                )}
                <div
                  className={`mx-auto h-full min-h-[488px] transition-all ${VIEWPORTS[viewport]}`}
                >
                  {generation ? (
                    executionMode === 'sandbox' ? (
                      <SandboxWorkbench
                    ...
                    <div className="h-full min-h-[488px] bg-black text-white flex items-center justify-center p-8">
                      <div className="max-w-md text-center">
                        ...
                        <h2 className="mt-3 text-3xl font-bold uppercase tracking-tighter">
                          Your app starts here.
                        </h2>
                        <p className="mt-3 text-sm leading-relaxed text-zinc-400">
                          Tell the assistant what you want to build. It will edit the files in this
                          project and the preview will update automatically.
                        </p>
                        <div className="mt-5 flex flex-wrap justify-center gap-2 text-[10px] uppercase tracking-widest text-zinc-500">
                          {executionMode === 'sandbox' ? (
                            <>
                              <span className="border border-zinc-900 px-2 py-1">
                                Next.js App Router
                              </span>
              ...
              <div className="h-full min-h-[520px] overflow-hidden border border-zinc-900 bg-black">
                <div className="border-b border-zinc-900 px-3 py-2 flex items-center gap-2 text-[10px] uppercase tracking-widest text-zinc-500">
                  ...
                  {generation &&
                    activeFile &&
                    /^src\/(?:App\.tsx|index\.css|(?:components|hooks|lib)\/[A-Za-z0-9_-]+\.(?:tsx|ts)|[A-Za-z0-9_-]+\.css)$/.test(
                      activeFile.path
          ...
          <div className="border-t border-zinc-900 xl:border-l xl:border-t-0">
              ...
              {files.length === 0 ? (
                ...
                files.map((file) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between gap-2 px-2 py-2 text-left text-xs transition-colors ${
            ...
            <div className="p-4 space-y-3 text-xs text-zinc-500">
              ...
              <div className="flex items-center justify-between gap-4">
                <span>Mode</span>
                <span className={autoApprove ? 'text-orange-500' : 'text-zinc-300'}>
            ...
            <div className="ml-auto flex items-center gap-1">
              {(['all', 'error', 'warn', 'log'] as const).map((level) => (
                <button
                  ...
                  className={`px-2 py-1 transition-colors disabled:cursor-not-allowed disabled:opacity-30 ${
                    consoleFilter === level
                      ? 'bg-white text-black'
                      : 'text-zinc-500 hover:text-white'
                  }`}
          ...
          <div key={project.id} className="grid gap-4 py-5 md:grid-cols-[1fr_auto] md:items-center">
                ...
                <span
                  className={
                    project.status === 'PUBLISHED'
                      ...
                      : 'text-[10px] uppercase tracking-widest text-zinc-600'
                  }
                >
                  {project.status}
                </span>
            ...
            <div className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-widest">
              <button
                ...
                className="border border-zinc-800 px-3 py-2 text-zinc-300 hover:border-zinc-600 hover:text-white"
              >
                Open
              </button>
              <button
                ...
                className="border border-zinc-800 px-3 py-2 text-zinc-500 hover:border-zinc-600 hover:text-white"
              >
                Rename
              </button>
              <button
                ...
                className="inline-flex items-center gap-2 border border-zinc-800 px-3 py-2 text-zinc-500 hover:border-zinc-600 hover:text-white"
              >
  ...
  return (
    <section className="max-w-3xl px-6 py-10">
      ...
      <p className="mt-2 max-w-xl text-sm leading-relaxed text-zinc-500">
        ...
        — restoring is itself undoable, so you never lose work. Keeps the last {MAX_HISTORY}{' '}
    ...
    <section className="max-w-5xl px-6 py-10">
      <div className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-500">
          Published playground apps show here. Open a project to inspect files, open the live app,
          or download the generated source.
        </p>
      </div>
      ...
      {projects.length === 0 ? (
        <div className="border border-zinc-900 p-8">
          ...
          <h2 className="mt-5 text-xl font-bold uppercase tracking-tighter">
            No published apps yet
          </h2>
          <p className="mt-2 text-sm text-zinc-500">
            Publish the current project to create the first app entry.
          </p>
          <button
            type="button"
            onClick={onPublish}
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-zinc-500">
                      <div>
                        ...
                        <div className="mt-1 text-zinc-300">
                          {project.deploymentProvider ?? 'preview'}
                        </div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-widest text-zinc-700">
                        ...
                        <div className="mt-1 text-zinc-300">
                          {project.deploymentState ?? 'published'}
                        </div>
                      </div>
                    </div>
  ...
  return (
    <section className="max-w-5xl px-6 py-10">
      <div className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-500">
          Publish the generated Vite app as an Agentbot preview, then push the same source to a
          GitLawb node as a real git repo.
        </p>
      </div>
      ...
      <div className="grid gap-px bg-zinc-900 md:grid-cols-[1fr_320px]">
        <div className="bg-black p-6">
          ...
          <h2 className="mt-4 text-2xl font-bold tracking-tighter">
        ...
        <div className="bg-black p-6">
          ...
          <div className="mt-4 border-t border-zinc-900 pt-4">
            ...
            <p className="mt-2 text-[10px] text-zinc-600">
              Saves a copy and opens the deploy flow.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <section className="max-w-6xl px-6 py-10">
      <div className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-500">
          Start with a pre-built template. Pick one, customise the prompt, and let OpenClaude build
          it.
        </p>
      </div>
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-1">
              {template.tags.map((tag) => (
                <span
                  key={tag}
                  className="border border-zinc-900 px-1.5 py-0.5 text-[9px] uppercase tracking-widest text-zinc-600"
                >
                  {tag}
                </span>
              ))}
