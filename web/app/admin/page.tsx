...
export default function AdminPage() {
  ...
  const [activities, setActivities] = useState<
    Array<{ type: string; message: string; timestamp: string; status?: string }>
  >([]);
  ...
  const performSync = async () => {
    if (
      !confirm(
        'CAUTION: This will reconcile plural backend tables with singular Prisma tables. Proceed?'
      )
    )
      return;
    setSyncing(true);
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center font-mono">
        ...
        <p className="animate-pulse uppercase tracking-[0.2em] text-[10px] text-zinc-500">
          Initializing Admin_Session...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono selection:bg-orange-500 selection:text-black">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-600 mt-1 uppercase tracking-tighter">
                Root_Access // {session?.user?.email}
              </p>
            </div>
          </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4 sm:py-8 flex flex-col sm:flex-row gap-4 sm:gap-8">
        {/* ─── Sidebar Navigation ────────────────────────────────────────────── */}
        <div className="w-full sm:w-64 shrink-0 space-y-1 flex sm:block overflow-x-auto sm:overflow-x-visible">
          <TabButton
        ...
        <div className="pt-8 space-y-4">
          <div className="px-4 py-2 border border-dashed border-zinc-800 rounded-sm">
            ...
            <div className="flex items-center gap-2 mb-1">
              <div
                className={`w-1.5 h-1.5 rounded-full ${
                  stats?.backendStatus === 'OK' ? 'bg-green-500' : 'bg-orange-500 animate-pulse'
                }`}
              />
              <span className="text-[10px] uppercase font-bold text-zinc-400">
                Railway_API: {stats?.backendStatus || '...'}
              </span>
            </div>
            <div className="flex items-center gap-2 mb-1">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-1.5 h-1.5 rounded-full ${
                  dbHealth?.summary.status === 'healthy' ? 'bg-green-500' : 'bg-yellow-500'
                }`}
              />
              <span className="text-[10px] uppercase font-bold text-zinc-400">
                DB_Status:{' '}
                {dbHealth?.summary.status === 'healthy'
                  ? 'Synced'
                  : dbHealth?.summary.status === 'legacy_present'
                    ? 'Legacy OK'
                    : 'Drift'}
              </span>
            </div>
          ...
          <Link
            href="/admin/invites"
            className="flex items-center gap-3 px-4 py-3 bg-zinc-900 border border-zinc-800 hover:border-orange-500/40 hover:bg-zinc-900/50 transition-all group"
          >
            <UserPlus className="w-4 h-4 text-zinc-500 group-hover:text-orange-500" />
            <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-400 group-hover:text-white">
              Issue_Invites
            </span>
          </Link>
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                ...
                <StatCard
                  ...
                  color={stats?.backendStatus === 'OK' ? 'text-green-400' : 'text-red-400'}
                  isString
                />
              </div>
                        ...
                        <tr key={agent.id} className="hover:bg-zinc-900/40 group transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2 text-zinc-500">
                              {agent.websocketUrl?.includes('railway.app') ? (
                                ...
                                <span className="text-zinc-400 font-mono italic text-[9px] truncate max-w-[120px]">
                                  {agent.websocketUrl}
                                </span>
                              ) : (
                                <span>SELF_HOST</span>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex justify-end gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                              <button
                                className="p-2 hover:bg-zinc-800 rounded-sm text-zinc-400 hover:text-white"
                                title="View Logs"
                              >
                                <Terminal className="w-3.5 h-3.5" />
                              </button>
                              <button
                                className="p-2 hover:bg-orange-950/30 rounded-sm text-zinc-400 hover:text-orange-400"
                      ...
                      <tr key={user.id} className="hover:bg-zinc-900/40 group transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {user.isAdmin && (
                              <span className="px-1.5 py-0.5 bg-orange-500/20 border border-orange-500/40 text-orange-500 text-[8px] font-bold uppercase">
                                ADMIN
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`font-bold uppercase ${
                              user.plan !== 'free' ? 'text-orange-500' : 'text-zinc-600'
                            }`}
                          >
                            {user.plan}
                          </span>
          ...
          {activeTab === 'integrity' && dbHealth && (
            <div className="space-y-6">
              <div
                className={`p-6 border-l-4 rounded-r-sm ${
                  dbHealth.summary.status === 'critical_drift'
                    ? 'bg-red-950/20 border-orange-500'
                    : dbHealth.summary.status === 'legacy_present'
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div className="bg-zinc-950 border border-zinc-800 p-6 space-y-6">
                  <h4 className="text-[10px] uppercase font-bold text-zinc-500 tracking-[0.2em] border-b border-zinc-900 pb-2">
                    Active_Models (Prisma)
                  </h4>
                  <div className="space-y-4">
                    ...
                    <IntegrityItem
                      ...
                      value={dbHealth.counts.activeDeployments}
                    />
                  </div>
                </div>
                ...
                <div className="bg-zinc-950 border border-zinc-800 p-6 space-y-6">
                  <h4 className="text-[10px] uppercase font-bold text-zinc-500 tracking-[0.2em] border-b border-zinc-900 pb-2">
                    Legacy_Drift (Postgres)
                  </h4>
                  <div className="space-y-4">
                    <DriftItem
                      label="Legacy_Users_Table"
                      detected={dbHealth.drift.hasLegacyUsers}
                    />
                    <DriftItem
                      label="Legacy_Agents_Table"
                      detected={dbHealth.drift.hasLegacyAgents}
                    />
                    <IntegrityItem
                      label="Ghost_Tables_Detected"
                      value={dbHealth.drift.ghostTables}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-zinc-900/30 border border-dashed border-zinc-800 p-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wider">
                    Sync the hot-tier (Postgres) with the warm-tier (Gitlawb facts).
                  </p>
                </div>
                <button
                  onClick={performSync}
                  disabled={syncing}
                  className="w-full sm:w-auto bg-white text-black px-6 py-2 text-xs font-bold uppercase tracking-widest hover:bg-orange-500 transition-colors disabled:opacity-50"
                >
...
function StatCard({
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 p-5 rounded-sm shadow-sm hover:border-zinc-700 transition-colors">
      <div className="text-[9px] text-zinc-600 uppercase font-bold tracking-[0.2em] mb-1">
        {label}
      </div>
...
function DriftItem({ label, detected }: { label: string; detected: boolean }) {
  return (
    <div className="flex items-center justify-between text-[11px]">
      <span className="text-zinc-500 font-bold uppercase tracking-wider">{label}:</span>
      {detected ? (
        <span className="text-orange-500 font-bold uppercase px-1.5 bg-orange-500/10">
          DETECTED
        </span>
      ) : (
