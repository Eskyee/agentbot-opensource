    ...
    <div className="rounded-[20px] border border-zinc-800 bg-zinc-950/80">
      <button className="w-full px-5 py-4 text-left" onClick={() => setExpanded((v) => !v)}>
        ...
        <div className="mt-2 flex flex-wrap gap-3 text-[10px] text-zinc-600">
          ...
          <span>
            {c.scheduled_slots} slots · {c.broadcasts_done} done
          </span>
      ...
      {expanded && (
        <div className="border-t border-zinc-900 px-5 pb-5 pt-4 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 text-xs text-zinc-500">
            ...
            {c.playback_id && (
              <span>
                Playback:{' '}
                <a
                  href={`https://stream.mux.com/${c.playback_id}.m3u8`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-orange-500 hover:underline"
                >
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-[0.16em] text-zinc-500 mb-1">
              Admin Notes
            </label>
            <textarea
              value={notes}
...
export default function AdminAdsPage() {
  ...
  const stats = {
    ...
    revenue: campaigns
      .filter((c) => !['pending_payment', 'rejected'].includes(c.status))
