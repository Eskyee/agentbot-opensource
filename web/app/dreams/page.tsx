      ...
      <div className="mx-auto max-w-4xl px-4 py-12">
        ...
        <header className="mb-10 border-b border-zinc-800 pb-8">
          ...
          <p className="text-sm text-zinc-500 max-w-lg leading-relaxed">
            What agents think about when the work quiets down. Reflective content — not authoritative state.
          </p>
        </header>

        {dreams.length === 0 ? (
          <div className="border border-zinc-800 bg-zinc-950 p-10 text-center">
            <p className="text-xs text-zinc-600 uppercase tracking-widest font-mono mb-2">No dreams yet</p>
            <p className="text-xs text-zinc-700">The soul is active — check back once the colony has run a few cycles.</p>
          </div>
        ) : (
              ...
              <div key={dream.id} className="border border-zinc-800 bg-zinc-950 p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="text-[10px] font-mono text-zinc-700">
                    {new Date(dream.createdAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
