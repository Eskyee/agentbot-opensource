...
function DeployFlowDiagram() {
  return (
    <svg viewBox="0 0 640 110" role="img" aria-label="Deploy flow: sign up, container boots in about 2 minutes, agent live 24/7" className="mt-6 w-full h-auto">
      {[
        ...
        <g key={step.label}>
          <rect x={step.x} y={30} width={180} height={56} fill="none" stroke={i === 2 ? '#f97316' : '#27272a'} strokeOpacity={i === 2 ? 0.6 : 1} />
          <text x={step.x + 90} y={54} textAnchor="middle" fill={i === 2 ? '#f97316' : '#fafafa'} fontSize="11" fontFamily="monospace" letterSpacing="1">{step.label}</text>
          <text x={step.x + 90} y={72} textAnchor="middle" fill="#71717a" fontSize="9" fontFamily="monospace">{step.sub}</text>
        </g>
      ))}
...
function ChannelsDiagram() {
  return (
    <svg viewBox="0 0 640 170" role="img" aria-label="One agent brain with shared memory connected to Telegram, Discord, and WhatsApp" className="mt-6 w-full h-auto">
      <rect x={10} y={50} width={200} height={70} fill="none" stroke="#f97316" strokeOpacity={0.6} />
      <text x={110} y={80} textAnchor="middle" fill="#fafafa" fontSize="11" fontFamily="monospace" letterSpacing="1">YOUR AGENT</text>
      <text x={110} y={98} textAnchor="middle" fill="#71717a" fontSize="9" fontFamily="monospace">one brain · shared memory</text>
      {[
        ...
        <g key={ch.label}>
          ...
          <text x={500} y={ch.y + 16} textAnchor="middle" fill="#a1a1aa" fontSize="10" fontFamily="monospace">{ch.label}</text>
          <text x={500} y={ch.y + 28} textAnchor="middle" fill="#52525b" fontSize="8" fontFamily="monospace">{ch.sub}</text>
        </g>
      ))}
    </svg>
  )
...
function AutomationLoopDiagram() {
  return (
    <svg viewBox="0 0 640 130" role="img" aria-label="Automation loop: trigger fires, agent thinks, acts with skills, reports back, repeats on schedule" className="mt-6 w-full h-auto">
      {[
        ...
        <g key={step.label}>
          ...
          <text x={step.x + 67} y={52} textAnchor="middle" fill="#fafafa" fontSize="10" fontFamily="monospace" letterSpacing="1">{step.label}</text>
          <text x={step.x + 67} y={68} textAnchor="middle" fill="#71717a" fontSize="8" fontFamily="monospace">{step.sub}</text>
          {i < 3 && (
            <>
              <line x1={step.x + 134} y1={56} x2={step.x + 158} y2={56} stroke="#3f3f46" strokeDasharray="4 4" />
              <polygon points={`${step.x + 158},52 ${step.x + 166},56 ${step.x + 158},60`} fill="#f97316" />
            </>
          )}
        </g>
      ))}
      <path d="M 563 82 L 563 105 L 77 105 L 77 84" fill="none" stroke="#3f3f46" strokeDasharray="4 4" />
      <polygon points="73,86 77,78 81,86" fill="#f97316" />
      <text x={320} y={118} textAnchor="middle" fill="#52525b" fontSize="8" fontFamily="monospace">repeats on your schedule — no babysitting</text>
    </svg>
  )
}
