...
console.log(reply.choices[0].message.content)`,
  ...
  useEffect(() => {
    ...
    loadConsole().catch((err) =>
      setError(err instanceof Error ? err.message : 'Console failed to load')
    );
  }, []);
    ...
    <main className="min-h-screen bg-black text-zinc-100">
      <section className="border-b border-zinc-900">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <Link
              href="/"
              className="font-mono text-xs uppercase tracking-[0.28em] text-orange-500"
            >
              Agentbot
            </Link>
            <div className="flex items-center gap-3 font-mono text-xs uppercase tracking-[0.18em] text-zinc-500">
              ...
              <Link href="/vercel-gateway/health" className="hover:text-white">
                Health
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div>
              ...
              <h1 className="mt-4 max-w-3xl text-4xl sm:text-5xl md:text-7xl font-semibold tracking-tight text-white">
                Every agent.
                <br />
                One <span className="text-orange-500">gateway.</span>
              </h1>
              ...
              <div className="mt-6 flex flex-wrap gap-3 font-mono text-xs uppercase tracking-[0.18em]">
                <a
                  href="#keys"
                  className="border border-orange-500 bg-orange-500 px-4 py-2 text-black hover:bg-orange-400"
                >
                  Generate a key →
                </a>
                <Link
                  href="/playground"
                  className="border border-zinc-800 px-4 py-2 text-zinc-400 hover:border-zinc-600 hover:text-white"
                >
              ...
              <div className="mt-8 overflow-x-auto">
                <div className="flex items-center gap-2 whitespace-nowrap font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-600">
                  <span className="border border-zinc-800 px-2 py-1 text-zinc-400">
                    your client
                  </span>
                  <span className="text-zinc-700">──▶</span>
                  <span className="border border-orange-500/40 px-2 py-1 text-orange-500">
                    agentbot /v1
                  </span>
                  <span className="text-zinc-700">──▶</span>
                  <span className="border border-zinc-800 px-2 py-1 text-zinc-400">
            ...
            <div className="border border-zinc-900 bg-zinc-950 p-5">
              <div className="flex items-center justify-between gap-4">
                ...
                <StatusDot
                  state={health ? (health.ok ? 'online' : 'degraded') : 'queued'}
                  pulse={health?.ok}
                />
              </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        ...
        <div className="border border-zinc-900 bg-zinc-950 p-5">
          ...
          <div className="mt-4">
            <Snippet
              text={snippets[snippetTab].split('\n')}
              prompt={false}
          ...
          <p className="mt-4 text-sm text-zinc-500">
            New to the gateway?{' '}
            <Link
              href="/blog/posts/opengateway-explained"
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 pb-10 sm:px-6 lg:grid-cols-4 lg:px-8">
        {[
      ...
      <section className="mx-auto grid max-w-6xl gap-px bg-zinc-900 px-4 pt-14 pb-10 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">
        {[
          [
        ...
        <div className="border border-zinc-900 bg-zinc-950 p-6 sm:p-8">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold text-white">
              Stop picking models. Send{' '}
              <code className="text-orange-500">model: &quot;auto&quot;</code>.
            </h2>
            <span className="border border-orange-500/40 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-widest text-orange-500">
              New
            </span>
          </div>
          ...
          <p className="mt-3 text-xs text-zinc-600">
            ...
            <code className="text-zinc-400">
              {'{ "priority": "cost" | "balanced" | "quality", "max_cost_usd": 0.01 }'}
            </code>{' '}
            — stripped before it reaches any provider.
          </p>
        </div>
      </section>
          ...
          <div className="border border-zinc-900 bg-zinc-950 p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Live from <code className="text-zinc-400">/v1/models</code> — send the id exactly
                  as listed. MiMo also accepts the short form.
                </p>
              </div>
                ...
                <>
                  <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {shown.map((id) => (
                      <div
                        key={id}
                        className="flex items-center justify-between gap-2 border border-zinc-900 bg-black px-3 py-2"
                      >
                        <code className="truncate font-mono text-xs text-zinc-300">{id}</code>
                        {id.endsWith(':free') && (
                          <span className="shrink-0 border border-emerald-500/40 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-widest text-emerald-400">
                            Free
                          </span>
                        )}
                      </div>
                  ...
                  {filtered.length > shown.length && (
                    <p className="mt-3 text-xs text-zinc-600">
                      Showing {shown.length} of {filtered.length} — refine the filter to narrow
                      down.
                    </p>
                  )}
                  {filtered.length === 0 && (
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 pb-12 sm:px-6 lg:grid-cols-2 lg:px-8">
        <div className="border border-zinc-900 bg-zinc-950 p-5">
          ...
          <div className="mt-4 divide-y divide-zinc-900">
            {(usage?.keys || []).map((key) => (
              <div key={key.id} className="flex items-center justify-between gap-4 py-3">
                ...
                <button
                  ...
                  className="font-mono text-xs uppercase tracking-[0.18em] text-zinc-500 hover:text-orange-400"
                >
                  Revoke
                </button>
              </div>
            ...
            {!usage?.keys?.length ? (
              <p className="py-4 text-sm text-zinc-500">No keys yet.</p>
            ) : null}
          </div>
        </div>
        ...
        <div className="border border-zinc-900 bg-zinc-950 p-5">
          ...
          <div className="mt-4 divide-y divide-zinc-900">
            ...
            {!usage?.byModel?.length ? (
              <p className="py-4 text-sm text-zinc-500">No gateway traffic yet.</p>
            ) : null}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-zinc-900">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-500">
            One key, one base URL — five OpenAI-style endpoints that the Agentbot stack runs on,
            open for yours too.
          </p>
          <div className="mt-8 grid gap-px bg-zinc-900 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <div key={endpoint} className="bg-zinc-950 p-5">
                ...
                <code className="mt-2 block break-all font-mono text-[11px] text-orange-500">
                  {endpoint}
                </code>
                <p className="mt-2 text-sm leading-6 text-zinc-500">{body}</p>
              </div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
          ...
          <div className="mt-8 grid gap-px bg-zinc-900 sm:grid-cols-3">
            {[
              [
                'Playground',
                'Every app built in the Playground streams its generation through this gateway — multi-file React apps, live.',
                '/playground',
              ],
              [
                'Agent containers',
              ...
              ['OpenClaw', 'The 24/7 agent runtime that powers all Agentbot agents.', '/openclaw'],
            ].map(([title, body, href]) => (
              <Link
                key={title}
                href={href}
              ...
              >
                <h3 className="text-sm font-semibold text-white group-hover:text-orange-500">
                  {title} →
                </h3>
                <p className="mt-2 text-sm leading-6 text-zinc-500">{body}</p>
              </Link>
      ...
      <section className="border-t border-zinc-900">
        <div className="mx-auto max-w-6xl px-4 py-12 text-center sm:px-6 sm:py-16 lg:px-8">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3 font-mono text-xs uppercase tracking-[0.18em]">
            <a
              href="#keys"
              className="border border-orange-500 bg-orange-500 px-5 py-2.5 text-black hover:bg-orange-400"
            >
              Open console →
            </a>
            <Link
              href="/blog/posts/opengateway-explained"
              className="border border-zinc-800 px-5 py-2.5 text-zinc-400 hover:border-zinc-600 hover:text-white"
            >
