  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-4 right-4 w-14 h-14 bg-brand-600 hover:bg-brand-500 rounded-full flex items-center justify-center text-white shadow-lg transition-all hover:scale-110 z-50"
      >
        {isOpen ? (
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-4 w-80 h-96 bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl flex flex-col z-50">
          {/* Header */}
          <div className="bg-brand-600 px-4 py-3 rounded-t-2xl flex items-center gap-2">
          ...
          >
            <input
              ...
              className="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-brand-500"
              disabled={loading}
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="px-3 py-2 bg-brand-600 text-white rounded-lg text-sm font-bold hover:bg-brand-500 disabled:opacity-50"
            >
              →
            </button>
          </form>
