  ...
  return (
    <div className="mt-10 border border-zinc-800 bg-zinc-950 p-4 sm:p-5 max-w-lg">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[10px] uppercase tracking-widest text-zinc-500">Live — your worker is active</span>
      </div>
      <div className="space-y-1 font-mono text-xs">
        {activities.slice(0, visible).map((a, i) => (
          <div
            key={i}
            className="flex items-start gap-2 py-1.5 opacity-0"
            style={{ animation: `fade-in-stagger 0.4s ease-out ${i * 0.08}s both` }}
          >
            <span className={`w-16 shrink-0 text-right ${channelColors[a.channel]}`}>
              {a.channel}
            </span>
            <span className="text-zinc-600 mt-px">→</span>
            <span className="text-zinc-300 flex-1 leading-snug">{a.action}</span>
            <span className="text-zinc-600 shrink-0 text-[10px] mt-px">{a.time}</span>
          </div>
        ))}
      </div>
      {visible >= activities.length && (
        <div className="mt-3 pt-3 border-t border-zinc-800 text-[10px] text-zinc-600 uppercase tracking-widest">
          This happens all day. Every day. Without you.
        </div>
      )}
    </div>
  )
