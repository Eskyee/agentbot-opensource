...
const DeployModal = dynamic(
  () => import('@/app/components/DeployModal').then((m) => ({ default: m.DeployModal })),
  { ssr: false }
);
      ...
      {successName && (
        <div className="mb-8 border border-green-500/30 bg-green-500/10 px-5 py-3 flex items-center gap-3">
          ...
          <span className="text-green-400 text-xs uppercase tracking-widest font-bold">
            {successName} deployed — redirecting...
          </span>
        </div>
      )}
      ...
      {templates.length === 0 ? (
        <div className="border border-zinc-800 bg-zinc-950 p-10 text-center">
          ...
          <p className="text-xs text-zinc-500 max-w-md mx-auto mb-6">
            The marketplace is being curated. Check back soon or deploy a custom agent from the
            dashboard.
          </p>
          <button
            ...
            >
              ...
              <div className="grid gap-2 grid-cols-2 mb-5">
                {template.skills.map((skill) => (
                  <div
                    key={skill}
                    className="text-[10px] uppercase tracking-widest border border-zinc-800 px-3 py-1.5 text-zinc-500"
                  >
                    {skill}
                  </div>
                ))}
