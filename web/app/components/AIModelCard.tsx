    ...
    <div className="bg-zinc-900 rounded-2xl p-6 border border-zinc-800">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
            <div
              className={`h-full ${config.reasoning === 0 ? 'bg-zinc-700' : 'bg-red-500'} rounded-full transition-all duration-500`}
              style={{ width: `${reasoningPercent}%` }}
            />
          </div>
          {config.reasoning === 0 && (
            <button className="w-full mt-3 py-2 text-xs font-bold bg-red-600/20 text-red-500 border border-red-500/30 rounded-lg hover:bg-red-600/30 transition-colors">
