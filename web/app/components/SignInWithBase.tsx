    ...
    <div className="space-y-4">
      ...
      {error && (
        <div className="text-red-400 text-sm text-center p-3 bg-red-900/20 rounded-lg border border-red-800">
          {error}
          <button onClick={() => { hasSignedRef.current = false; handleSignIn(); }} className="block mx-auto mt-2 text-xs text-brand-400 hover:text-brand-300">
            Try again
          </button>
        </div>
      )}
      ...
      >
        <div className="w-4 h-4 bg-brand-600 rounded-sm" />
        Sign in with Base
      </button>
    </div>
  );
