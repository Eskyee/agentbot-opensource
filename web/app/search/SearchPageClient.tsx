      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-10">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-zinc-400">
            Search the user-facing help, docs, developer pages, runtime guides, and key architecture posts.
          </p>
        </div>
        ...
        <div className="border border-zinc-800 bg-zinc-950 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-[10px] uppercase tracking-widest text-zinc-600">
            {['ffmpeg', 'basefm', 'runtime', 'missing dependencies', 'verify', 'gitlawb'].map((term) => (
              <button
                key={term}
        ...
        <div className="mt-8">
          {loading ? (
            ...
            <p className="text-sm text-zinc-500">No matches yet. Try broader terms like `runtime`, `docs`, `stream`, or `wallet`.</p>
          ) : (
            <div className="grid gap-px bg-zinc-800">
              {results.map((result) => (
                <Link key={result.id} href={result.href} className="bg-black p-5 transition-colors hover:bg-zinc-950">
                  ...
                  <p className="mt-3 text-xs uppercase tracking-widest text-orange-400">{result.href}</p>
                </Link>
              ))}
            </div>
          )}
