    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          <p className="text-zinc-600 text-xs uppercase tracking-widest font-mono mb-3">Colony not found</p>
          <Link href="/colony" className="text-zinc-400 hover:text-white text-xs uppercase tracking-widest transition-colors">
            ← Back to colonies
          </Link>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="border border-zinc-800 bg-zinc-950 p-6 mb-6">
          <div className="flex items-start justify-between gap-6 flex-wrap">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className={`w-2 h-2 rounded-full inline-block ${degraded ? 'bg-red-500' : 'bg-green-500'}`} />
                <h1 className="text-2xl font-bold uppercase tracking-tighter text-white">
                  {root?.designation ?? 'Agentbot Colony'}
                </h1>
              </div>
              <p className="text-xs text-zinc-600 font-mono mt-1">
                {tree?.colony_size ?? 0} nodes · avg fitness {tree?.avg_fitness ?? 0}% · {degraded ? 'degraded' : 'healthy'}
              </p>
              {root?.soul?.mode && (
            ...
            <div className="flex flex-col gap-2 text-right shrink-0">
              {root?.wallet_balance && (
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-zinc-600 font-mono">Wallet</p>
              ...
              {root?.colony && (
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-zinc-600 font-mono">Rank</p>
                  <p className="text-sm font-mono text-white">#{root.colony.rank} of {root.colony.colony_size}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {activePlan && (
            <div className="mt-5 border-t border-zinc-800 pt-4">
              <p className="text-[10px] uppercase tracking-widest text-zinc-600 font-mono mb-1">Active plan</p>
              <div className="flex items-center gap-3">
                <div className="flex-1 h-1 bg-zinc-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-amber-500 transition-all"
                    style={{ width: `${(activePlan.current_step / Math.max(activePlan.total_steps, 1)) * 100}%` }}
                  />
                </div>
                ...
                return (
                  <div key={agent.id} className="border border-zinc-800 bg-zinc-950 p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="text-sm font-bold text-white uppercase tracking-widest">{agent.name}</h3>
                        <p className="text-[10px] font-mono text-zinc-600 mt-0.5">{agent.specialization ?? 'general'} · gen {agent.generation ?? 1}</p>
                      </div>
                      <span className={`text-[10px] uppercase tracking-widest font-bold ${MOOD_COLORS[mood] ?? 'text-zinc-600'}`}>
                        {mood}
                      </span>
                    </div>
                    ...
                    <div className="mb-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-mono">Fitness</span>
                        <span className="text-[10px] font-mono text-zinc-400">{agent.fitness ?? 0}%</span>
                      </div>
                    ...
                    <div className="space-y-1">
                      {agent.walletAddress && agent.walletAddress !== '0x0000000000000000000000000000000000000000' && (
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-mono">Wallet</span>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-mono">Status</span>
                        <span className={`text-[10px] font-mono ${agent.status === 'active' ? 'text-green-400' : agent.status === 'culling' ? 'text-red-400' : 'text-amber-400'}`}>
                          {agent.status}
                        </span>
                      </div>
                      {agent.children > 0 && (
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-mono">Spawned</span>
                          <span className="text-[10px] font-mono text-zinc-400">{agent.children} nodes</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          {root?.soul && (
            <div className="border border-zinc-800 bg-zinc-950 p-5">
              <h3 className="text-[10px] uppercase tracking-widest text-zinc-600 mb-4 font-mono">Soul state</h3>
          ...
          {/* Quick actions */}
          <div className="border border-zinc-800 bg-zinc-950 p-5">
            <h3 className="text-[10px] uppercase tracking-widest text-zinc-600 mb-4 font-mono">Actions</h3>
        ...
        <div className="mt-4 border border-zinc-800 bg-zinc-950 p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[10px] uppercase tracking-widest text-zinc-600 font-mono">Babies</h3>
            <Link
              href="/buddies"
          ...
          <p className="text-xs text-zinc-600 leading-relaxed">
            Agent companions hatch and grow alongside your colony. Each baby bonds with an agent — feeding off its activity, earning XP, and evolving through stages.
          </p>
          <Link
            href="/buddies"
