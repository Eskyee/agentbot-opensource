...
function AgentLineageNode({
  agent,
  allAgents,
  depth = 0,
}: {
  agent: ColonyAgent;
  allAgents: ColonyAgent[];
  depth?: number;
}) {
  const children = allAgents.filter(
    (a) => a.parent === agent.walletAddress || a.parent === agent.id
  );
      ...
      <div className="border border-zinc-800 bg-zinc-950 p-3 mb-2">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            {agent.version && (
              <span className="text-[10px] font-mono text-zinc-500">v{agent.version}</span>
            )}
          </div>
        ...
        <div className="mt-1 flex items-center gap-2">
          <span className="text-[9px] font-mono text-zinc-500 truncate">{agent.walletAddress}</span>
  ...
  return (
    <div className="border border-zinc-800 bg-zinc-950 p-4 mb-8">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] font-mono text-red-500 uppercase">Soul State</span>
        <StatusPill
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-4 text-[10px] font-mono">
        <div>
          <div className="text-zinc-500 mb-0.5">Cycles</div>
          <div className="text-white">{soul.total_cycles}</div>
        </div>
        {soul.free_energy && (
          <div>
            <div className="text-zinc-500 mb-0.5">Free Energy</div>
        ...
        {soul.brain && (
          <div>
            <div className="text-zinc-500 mb-0.5">Brain</div>
        ...
        {soul.transformer && (
          <div>
            <div className="text-zinc-500 mb-0.5">Transformer</div>
            <div className="text-white">
              {(soul.transformer.param_count / 1000).toFixed(0)}K params
            </div>
        ...
        {soul.active_plan && (
          <div>
            ...
            <div className="text-white">
              {soul.active_plan.current_step}/{soul.active_plan.total_steps}
            </div>
  ...
  const action = (
    <div className="flex flex-wrap gap-2">
      <a
        ...
        className="border border-zinc-700 hover:border-red-500 text-red-500 text-[10px] font-bold uppercase tracking-widest py-2 px-4 flex items-center gap-2 transition-colors"
      >
      ...
      <DashboardContent>
        {lastFetch && (
          <div className="mb-4 flex items-center gap-4">
            ...
            {error && data && (
              <span className="text-[10px] text-yellow-500 font-mono">⚠ {error}</span>
            )}
          </div>
        )}
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <div className="flex gap-3">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="border border-zinc-700 hover:border-red-500 text-red-500 text-[10px] font-bold uppercase tracking-widest py-2 px-4 flex items-center gap-2"
              >
                <ExternalLink className="w-3 h-3" />
                Borg Dashboard
              </a>
        ...
        {data && (
          <>
            ...
            {/* Stats */}
            <div className="grid gap-px bg-zinc-900 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-8">
              <div className="bg-zinc-950 border border-zinc-800 p-4">
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">
                  Colony Size
                </div>
                <div className="text-2xl font-bold tracking-tight">{data.colony_size}</div>
                <div className="text-[10px] text-zinc-500">{activeAgents} agents active</div>
              </div>
              <div className="bg-zinc-950 border border-zinc-800 p-4">
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">
                  Avg Fitness
                ...
                <div className="text-[10px] text-zinc-500">colony health</div>
              </div>
              <div className="bg-zinc-950 border border-zinc-800 p-4">
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">
                  Fittest
                ...
                <div className="text-[10px] text-zinc-500">
                  {data.fittest?.fitness ?? 0}% fitness
                </div>
              </div>
              <div className="bg-zinc-950 border border-zinc-800 p-4">
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">
                ...
                <div className="text-[10px] text-zinc-500">below 40% fitness</div>
              </div>
            </div>
            ...
            {data.root.wallet_balance && (
              <div className="inline-block border border-zinc-800 bg-zinc-950 p-3 mb-6">
                <span className="text-[10px] font-mono text-zinc-500 mr-2 uppercase tracking-widest">
                  {data.root.designation || 'Borg Soul'}
                </span>
                <span className="text-sm font-mono text-white">
                  {data.root.wallet_balance.formatted}
                </span>
                <span className="text-[10px] font-mono text-zinc-400 ml-1">
                  {data.root.wallet_balance.token}
                </span>
              </div>
            )}
            {data.root.serviceUrl && (
              <div className="mb-6 flex flex-wrap items-center gap-3 text-[10px] font-mono">
                ...
                {data.detail ? <span className="text-zinc-500">{data.detail}</span> : null}
              </div>
            )}
            ...
            {view === 'tree' ? (
              <div className="space-y-2">
                <h2 className="text-[10px] uppercase tracking-widest text-zinc-500 mb-4">
                  Lineage Tree
                </h2>
                {data.agents
                  .filter((a) => !a.parent)
                ...
                <div className="border border-zinc-800 overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="bg-zinc-950 text-[10px] uppercase tracking-widest text-zinc-500 border-b border-zinc-800">
