    ...
    <div className="mt-[4rem] min-h-screen bg-black text-white">
      ...
      <div className="px-6 py-5 border-b border-zinc-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-zinc-500 bg-zinc-900 border border-zinc-700 rounded-full px-3 py-0.5">{ITEMS.length} items</span>
        </div>
        <span className="text-xs text-zinc-500 font-mono">Updated 2026-03-14</span>
      </div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-1.5">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => toggleStar(item.id)} className="transition-colors">
                        <Star className={`h-4 w-4 ${starred.has(item.id) ? 'text-yellow-400 fill-yellow-400' : 'text-zinc-600 hover:text-zinc-400'}`} />
                      </button>
                    </div>
                  </div>
                  <p className="text-xs text-zinc-400 leading-relaxed mb-3">{item.summary}</p>
