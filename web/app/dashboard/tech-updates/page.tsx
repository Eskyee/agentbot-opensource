...
const ITEMS: TechItem[] = [
  {
    ...
    summary:
      'Anthropic ships extended thinking to production. 200K context window, improved tool use reliability, and hybrid reasoning mode that blends fast and deep thinking per token budget.',
    date: '2026-03-14',
    source: 'Anthropic',
  ...
  },
  {
    ...
    summary:
      'Model Context Protocol adds resource subscriptions so servers can push updates to clients without polling. Key for real-time agent memory and event-driven pipelines.',
    date: '2026-03-13',
  ...
  },
  {
    ...
    summary:
      'Vercel announces built-in KV at the edge — zero cold starts, 1ms global reads. Replaces the need for external Redis for session and rate-limiting use cases.',
    date: '2026-03-12',
    source: 'Vercel',
  ...
  },
  {
    ...
    summary:
      'OWASP updates LLM security guidance with structured output enforcement, tool call sandboxing, and multi-agent trust boundary recommendations.',
    date: '2026-03-11',
    source: 'OWASP',
    url: 'https://owasp.org',
    impact: 'high',
  },
  {
    ...
    summary:
      'Prisma 6 ships native Turso / libSQL adapter. SQLite at the edge with branching and embedded replicas — critical for local-first agent architectures.',
    date: '2026-03-10',
    source: 'Prisma',
    url: 'https://prisma.io',
    impact: 'medium',
  },
  {
    ...
    summary:
      'Parallel function calls, typed return schemas, and auto-retry for malformed tool outputs. Reduces agent reliability engineering by ~60% in early benchmarks.',
    date: '2026-03-09',
    source: 'OpenAI',
  ...
  },
  {
    ...
    summary:
      'PPR ships stable — static shell + streaming dynamic holes. Eliminates the loading spinner anti-pattern for authenticated dashboards.',
    date: '2026-03-08',
  ...
  },
  {
    ...
    summary:
      'Google publishes Agent-to-Agent authentication spec with JWT + DID-based identity. Enables cryptographically verified inter-agent messaging across providers.',
    date: '2026-03-07',
...
const CATEGORY_META: Record<Category, { label: string; icon: LucideIcon; color: string }> = {
  ...
  models: { label: 'Models', icon: Cpu, color: 'text-orange-500' },
  infra: { label: 'Infra', icon: Zap, color: 'text-orange-500' },
...
function buildOpenClawItems(soulData: SoulData | null, fetchError: boolean): TechItem[] {
    ...
    return [
      {
        ...
        summary:
          'Failed to retrieve OpenClaw soul data from /api/colony/status. Check connectivity and retry.',
        category: 'openclaw',
        date: today(),
        source: 'Borg Soul',
        url: OPENCLAW_URL,
  ...
  if (!soulData) {
    return Array.from({ length: 3 }, (_, i) => ({
      id: `oc-skeleton-${i}`,
      title:
        i === 0 ? 'Loading soul data...' : i === 1 ? 'Loading neural core...' : 'Loading goals...',
  ...
  return (
    <DashboardShell>
      <DashboardHeader
        ...
        action={
          active === 'openclaw' && lastRefresh ? (
            <span className="flex items-center gap-1.5 text-[10px] text-zinc-500 font-mono uppercase tracking-widest">
      ...
      <DashboardContent className="space-y-5">
        {/* Category filter */}
        <div className="flex flex-wrap gap-px bg-zinc-900">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 text-[10px] font-bold uppercase tracking-widest transition-colors ${
                  active === cat
                    ...
                    : 'bg-zinc-950 border border-zinc-800 text-zinc-500 hover:text-zinc-300 hover:border-zinc-600'
                }`}
              >
        ...
        <div className="space-y-px bg-zinc-800">
          {filtered.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`bg-zinc-950 border border-zinc-800 p-5 flex gap-4 ${
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-1.5">
                    <h3
                      className={`text-sm font-bold uppercase tracking-tight leading-tight ${
                        isSkeleton ? 'text-zinc-500' : 'text-white'
                      }`}
                    >
                      {item.title}
                    </h3>
                    <div className="flex items-center gap-2 shrink-0">
                      <StatusPill
                        status={IMPACT_STATUS[item.impact]}
                        label={item.impact}
                        size="sm"
                      />
                  ...
                  <p
                    className={`text-xs leading-relaxed mb-3 ${
                      isSkeleton ? 'text-zinc-800 select-none' : 'text-zinc-500'
                    }`}
                  >
                    {item.summary}
                  </p>
                  <div className="flex items-center gap-3 text-[10px] text-zinc-500">
                    ...
                    <a
                      ...
                      className="ml-auto flex items-center gap-1 text-zinc-400 hover:text-white transition-colors uppercase tracking-widest font-bold"
                    >
