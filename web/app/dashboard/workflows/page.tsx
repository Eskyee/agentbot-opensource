...
const nodeTypes = [
  {
    ...
    desc: 'Webhook, cron, message, or event',
  },
  {
    type: 'agent',
    label: 'Agent',
    icon: Bot,
    color: 'text-orange-400',
    desc: 'Run an agent with a prompt',
  },
  {
    type: 'condition',
    label: 'Condition',
  ...
  },
  {
    ...
    desc: 'API call, tool use, or side effect',
  },
  {
    type: 'delay',
    label: 'Delay',
  ...
  },
  {
    ...
    icon: MessageSquare,
    color: 'text-sky-400',
    desc: 'Send message, webhook, or store',
  },
];
    ...
    >
      ...
      {(node.config as Record<string, unknown>)?.prompt ? (
        <div className="text-[10px] text-zinc-500 mt-1 line-clamp-2">
          {String((node.config as Record<string, unknown>).prompt)}
        </div>
      ) : null}
    </div>
  );
        ...
        <div className="relative group">
          ...
          >
            <div className="text-center">
              <Plus className="h-5 w-5 text-zinc-500 mx-auto mb-1" />
              <span className="text-[10px] uppercase tracking-widest text-zinc-500">Add Step</span>
            </div>
          </button>
        </div>
      </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center">
            ...
            <p className="text-xs text-zinc-500 max-w-sm">
              Drag triggers, agents, conditions, and actions to create automated workflows. Your
              agents will execute them on autopilot.
            </p>
          </div>
        </div>
      )}
...
function NodeConfigPanel({
  ...
  return (
    <div className="border-t border-zinc-900 bg-zinc-950 p-4 space-y-4">
      ...
      <div className="text-[10px] text-zinc-500">{nodeType?.desc}</div>
    </div>
  );
}
          ...
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-zinc-950 border border-zinc-700 w-full max-w-md p-6">
              ...
              <input
                ...
                autoFocus
                onKeyDown={(e) =>
                  e.key === 'Enter' && newName.trim() && createMutation.mutate(newName)
                }
              />
        ...
        <div className="flex gap-px bg-zinc-900 h-full">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            {active ? (
              <>
                ...
                />
                {/* Node type palette */}
                <div className="border-t border-zinc-900 bg-zinc-950 px-4 py-3 flex gap-2 overflow-x-auto">
                  {nodeTypes.map((nt) => {
                    const Icon = nt.icon;
                    return (
                      <button
          ...
          {selectedNodeData && (
            <div className="w-72 bg-zinc-950 border-l border-zinc-800 overflow-y-auto shrink-0">
              <NodeConfigPanel node={selectedNodeData} onUpdate={handleUpdateNode} />
            </div>
          )}
        </div>
      </DashboardContent>
