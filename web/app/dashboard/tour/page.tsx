...
export default function TourManagementPage() {
  ...
  const totalRevenue = confirmedDates.reduce(
    (acc, d) => acc + parseFloat(d.fee.replace('$', '')),
    0
  );
  ...
  return (
    <DashboardShell>
      ...
      <DashboardContent className="space-y-6">
        <p className="text-xs text-zinc-500">Manage your upcoming tour dates and bookings.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-900">
          ...
          <div className="border border-zinc-800 bg-zinc-950 p-5">
            <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">Pending</div>
        ...
        <div>
          ...
          <div className="space-y-px bg-zinc-800">
            {tourDates.map((td) => (
              <div
                key={td.id}
                className="bg-zinc-950 border border-zinc-800 flex items-center justify-between p-4 hover:bg-zinc-900/30 transition-colors"
              >
                <div className="flex items-center gap-4">
                  <div className="text-left min-w-[60px]">
                    <div className="text-lg font-mono font-bold">{new Date(td.date).getDate()}</div>
                    <div className="text-[10px] text-zinc-500 uppercase tracking-widest">
                      {new Date(td.date).toLocaleDateString('en', { month: 'short' })}
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="text-right hidden sm:block">
                    ...
                    {td.ticketsSold > 0 && (
                      <div className="text-[10px] text-zinc-500">
                        {td.ticketsSold}/{td.capacity} tickets
                      </div>
                    )}
                  </div>
        ...
        <div className="border border-zinc-800 bg-zinc-950 p-8 text-left">
          <Map className="h-6 w-6 text-zinc-500 mb-3" />
