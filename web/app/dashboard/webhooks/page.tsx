...
import {
  Webhook, Plus, Trash2, ExternalLink, Zap, Github, CreditCard,
  MessageSquare, Globe, Bell, CheckCircle, XCircle, Clock,
} from 'lucide-react'
...
const templates: WebhookTemplate[] = [
  ...
  },
  {
    id: 'github-push',
    name: 'GitHub Push',
    description: 'Agent reacts to code pushes. Auto-review, deploy notifications, changelog generation, or issue updates.',
    provider: 'GitHub',
    icon: Github,
    color: 'text-zinc-300',
    event: 'push',
    payload: '{"ref": "refs/heads/main", "commits": [{"message": "feat: new feature", "author": {"name": "dev"}}]}',
    docsUrl: 'https://docs.github.com/en/webhooks',
  },
  {
    id: 'github-issue',
    name: 'GitHub Issue',
    description: 'Agent triages new issues, assigns labels, or responds with helpful context when issues are opened.',
    provider: 'GitHub',
    icon: Github,
    color: 'text-zinc-300',
    event: 'issues.opened',
  ...
  },
  {
    id: 'discord-message',
    name: 'Discord Event',
    description: 'Trigger on Discord events — new messages, reactions, member joins. Route to your agent for automated responses.',
    provider: 'Discord',
    icon: MessageSquare,
  ...
  },
  {
    ...
    description: 'Agent monitors mentions and replies automatically. Social media automation on autopilot.',
    provider: 'Twitter/X',
    icon: Globe,
    color: 'text-sky-400',
    event: 'mention',
  ...
  },
  {
    id: 'custom',
    name: 'Custom Webhook',
    description: 'Build your own integration. Point any service at your agent endpoint with a custom payload.',
    provider: 'Custom',
    icon: Zap,
              ...
              <div className="space-y-4 max-w-lg">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-black border border-zinc-800 px-3 py-2 text-sm text-white font-mono focus:border-orange-500 focus:outline-none"
                    placeholder="https://your-agent.up.railway.app/webhook/incoming"
                  />
                </div>
                <div className="border border-zinc-800 bg-black p-3">
                  <div className="text-[10px] text-zinc-600 uppercase tracking-widest mb-1">Event</div>
                  <code className="text-xs text-orange-400 font-mono">{selectedTemplate.event}</code>
                </div>
                <div className="border border-zinc-800 bg-black p-3">
                  <div className="text-[10px] text-zinc-600 uppercase tracking-widest mb-1">Example Payload</div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={cn(
                      ...
                      (!webhookName || !webhookUrl) ? 'opacity-40 cursor-not-allowed' : 'hover:bg-zinc-200'
                    )}
                  >
                    {createMutation.isPending ? 'Creating...' : 'Create Webhook'}
                  </button>
                  <button
                    onClick={() => { setSelectedTemplate(null); setShowCreate(false) }}
        ...
        <div className="border border-zinc-800 bg-zinc-950 p-5">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-sm font-bold text-white uppercase tracking-tight mb-2">No webhooks yet</h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-zinc-600 font-mono truncate mt-0.5">{wh.url}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="text-[10px] text-zinc-600 uppercase tracking-widest">Event</div>
                        <div className="text-[10px] font-mono text-zinc-400">{wh.event}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] text-zinc-600 uppercase tracking-widest">Last</div>
            ...
            {templates.map((t) => {
              ...
              return (
                <div key={t.id} className="bg-zinc-950 p-5">
                  <div className="flex items-center gap-2 mb-2">
                    <Icon className={cn('h-4 w-4', t.color)} />
                    <span className="text-xs font-bold text-white uppercase tracking-tight">{t.name}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {t.docsUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[9px] text-zinc-600 hover:text-orange-400 transition-colors"
                      >
                        Docs →
                      </a>
                    )}
