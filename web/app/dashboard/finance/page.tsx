...
export default function FinancePage() {
  ...
  const aiCost =
    costs
      ?.filter((c: any) => c.category === 'ai_metric')
      .reduce((sum: number, c: any) => sum + Number(c.total_spend), 0) ?? 0;
  const coordinationRevenue =
    (costs?.filter((c: any) => c.category === 'agent_message').length ?? 0) * 0.01;
  const portfolioValue =
    balances?.reduce((sum: number, b: any) => sum + Number(b.value || 0), 0) ?? 0;

  return (
    <DashboardShell>
      <DashboardHeader
        title="Finance & Profit"
        subtitle="Fleet costs, AI spend, and portfolio overview"
      ...
      <DashboardContent>
        ...
        <div className="grid gap-px bg-zinc-900 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-8">
          <div className="bg-zinc-950 border border-zinc-800 p-5">
            ...
            <div className="text-[10px] text-zinc-500 mt-1">all agents</div>
          </div>
          <div className="bg-zinc-950 border border-zinc-800 p-5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] uppercase tracking-widest text-zinc-500">
          ...
          <div className="bg-zinc-950 border border-zinc-800 p-5">
            ...
            <div className="text-[10px] text-zinc-500 mt-1">bankr holdings</div>
          </div>
        </div>

        {/* Charts & Tables */}
        <div className="grid gap-px bg-zinc-900 grid-cols-1 sm:grid-cols-2">
        ...
        {/* Wallet & Treasury */}
        <div className="grid gap-px bg-zinc-900 grid-cols-1 sm:grid-cols-2 mt-8">
          <div className="bg-zinc-950 border border-zinc-800 p-6">
