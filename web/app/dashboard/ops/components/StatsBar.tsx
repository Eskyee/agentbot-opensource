...
function StatBlock({
  ...
  return (
    <div className="flex-1 min-w-0 px-3 py-2.5 bg-zinc-950 border border-zinc-800">
      <div className="text-[9px] uppercase tracking-[0.18em] text-zinc-600 mb-1">{label}</div>
...
export function StatsBar({ stats, loading }: StatsBarProps) {
  ...
  return (
    <div className="grid grid-cols-3 sm:grid-cols-5 gap-px bg-zinc-800 mb-4 border border-zinc-800">
      ...
      <StatBlock
        label="Throughput"
        value={stats.throughput.callsPerMin > 0 ? `${formatNumber(stats.throughput.callsPerMin)}/min` : '—'}
      ...
      <StatBlock
        ...
        sub={stats.verifiedFacts.mirrorLag > 0 ? `lag ${stats.verifiedFacts.mirrorLag}ms` : 'mirror offline'}
        color="text-green-400"
      ...
      <StatBlock
        label="Errors"
        value={stats.errors.percent > 0 ? `${String(stats.errors.percent).padStart(4, '0')}%` : '00.00%'}
      ...
      <StatBlock
        ...
        sub={stats.spend24h.budgetPercent > 0 ? `${stats.spend24h.budgetPercent}% of budget` : 'no spend'}
