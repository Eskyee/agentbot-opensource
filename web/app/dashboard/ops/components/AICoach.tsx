  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 flex flex-col" style={{ height: 'min(500px, 60vh)' }}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`text-xs font-mono ${msg.role === 'user' ? 'text-zinc-400' : 'text-zinc-300'}`}>
            <span className={`text-[10px] uppercase tracking-widest mr-2 ${msg.role === 'user' ? 'text-blue-500' : 'text-green-500'}`}>
              {msg.role === 'user' ? '>' : '◄'}
            </span>
      ...
      <form onSubmit={handleSubmit} className="border-t border-zinc-800 p-2">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-zinc-900 border border-zinc-700 rounded-none px-2 py-1.5 text-xs text-zinc-300 font-mono placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500"
            disabled={loading}
          />
          <button
            type="submit"
