  ...
  return (
    <DashboardShell>
      ...
      <DashboardContent className="space-y-8">
        {lastGenerated && (
          <p className="text-[10px] text-zinc-500 font-mono">Generated at {lastGenerated}</p>
        )}

        {loading && !data ? (
          <div className="flex flex-col py-20 gap-4 items-center">
            <RefreshCw className="h-6 w-6 text-emerald-400 animate-spin" />
            <p className="text-zinc-500 text-xs uppercase tracking-widest">Scanning competitors…</p>
          </div>
        ) : (
          <>
            {/* Competitor grid */}
            <section>
              <h2 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4">
                Competitive Landscape — Live Status
              </h2>
                ...
                {data?.competitors.map((c) => {
                  ...
                  return (
                    <div key={c.name} className="bg-zinc-950 border border-zinc-800">
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            <span className="text-[10px] text-zinc-500 border border-zinc-800 px-2 py-0.5 font-mono">
                              {c.price}
                            </span>
                          </div>
                      ...
                      {isOpen && (
                        <div className="px-5 pb-4 border-t border-zinc-900 pt-3">
            ...
            <section>
              ...
              <div className="space-y-px bg-zinc-800">
                {data?.signals.map((sig) => (
                  <div
                    key={sig.id}
                    className="bg-zinc-950 border border-zinc-800 flex items-start gap-4 px-5 py-3"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-900">
                {data?.opportunities.map((opp) => (
                  <div key={opp.title} className="bg-zinc-950 border border-zinc-800 p-5">
                    <div className="text-sm font-bold uppercase tracking-tight mb-1.5">
                      {opp.title}
                    </div>
