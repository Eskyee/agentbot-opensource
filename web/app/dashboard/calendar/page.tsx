        ...
        {connected ? (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <button
                onClick={prevMonth}
                className="p-2 border border-zinc-800 hover:border-zinc-600 text-zinc-400 hover:text-white"
              >
                <ChevronLeft />
              </button>
            ...
            <div className="grid grid-cols-7 gap-px bg-zinc-900">
              {dayNames.map((day) => (
                <div
                  key={day}
                  className="bg-zinc-950 text-left text-zinc-500 py-2 px-2 text-[10px] uppercase tracking-widest"
                >
                  {day}
                </div>
              ))}
            ...
            <div className="grid grid-cols-7 gap-px bg-zinc-900">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div
                    key={day}
                    className={`h-24 bg-zinc-950 p-2 ${
                      isToday ? 'border border-white' : 'border border-zinc-800'
                    }`}
                  >
                    <div
                      className={`text-xs ${isToday ? 'text-white font-bold' : 'text-zinc-500'}`}
                    >
                      {day}
                    </div>
                    {dayEvents.slice(0, 2).map((event: any, idx: number) => (
                      <div
                        key={idx}
                        className="text-[10px] bg-orange-500/10 text-orange-400 truncate border border-orange-500/20 px-1 mt-1 py-0.5"
                      >
                        {event.summary}
                      </div>
                    ))}
            ...
            <div className="mt-8">
              ...
              <div className="space-y-px bg-zinc-800">
                ...
                {events.length === 0 && (
                  <div className="bg-zinc-950 text-zinc-500 py-8 text-center text-xs">
                    No upcoming events
                  </div>
                )}
              </div>
          ...
          <div className="py-16 text-center">
            <svg
              className="h-16 w-16 text-zinc-500 mx-auto mb-6"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1}
            ...
            <h2 className="text-sm font-bold tracking-tight uppercase mb-2">
              Connect Your Calendar
            </h2>
            <p className="text-xs text-zinc-500 mb-8 max-w-md mx-auto">
              Sync with Google Calendar to schedule events, manage availability, and let your agent
              handle bookings automatically.
            </p>
            <button
              onClick={connectCalendar}
