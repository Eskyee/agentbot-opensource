...
export default function InvoicePage() {
  ...
  const printInvoice = (inv: Invoice) => {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${inv.invoiceNumber}</title></head><body style="font-family:system-ui,sans-serif;max-width:720px;margin:40px auto;color:#111">
      <div style="display:flex;justify-content:space-between;align-items:flex-start"><div><h1 style="margin:0">Invoice ${inv.invoiceNumber}</h1><div style="color:#666;text-transform:uppercase;font-size:12px;letter-spacing:.1em;margin-top:4px">${inv.status}</div></div><div style="text-align:right;color:#666;font-size:13px">Due ${inv.dueDate || '—'}</div></div>
    ...
    <main className="min-h-screen bg-black text-white font-mono pt-14">
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="space-y-2">
          ...
          {invoices.map((inv) => (
            <div key={inv.id} className="rounded-xl border border-zinc-800 bg-zinc-950 p-4 flex items-center gap-4 hover:border-zinc-700 transition-colors">
              <button onClick={() => setSelected(inv)} className="flex-1 flex items-center gap-4 text-left min-w-0">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-zinc-500 mt-1 truncate">{inv.clientName || 'No client'}{inv.clientEmail ? ` · ${inv.clientEmail}` : ''}</div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-lg font-bold">{cur(inv.currency)}{(inv.total || 0).toFixed(2)}</div>
                  <div className="text-[10px] text-zinc-600">Due {inv.dueDate}</div>
                </div>
              </button>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => setSelected(inv)} title="Open" className="p-2 text-zinc-600 hover:text-white transition-colors"><Eye className="h-4 w-4" /></button>
                <button onClick={() => printInvoice(inv)} title="Print / Download" className="p-2 text-zinc-600 hover:text-white transition-colors"><FileText className="h-4 w-4" /></button>
                <button
                  onClick={() => { if (confirm(`Delete invoice ${inv.invoiceNumber}?`)) postInvoiceAction('delete', inv) }}
                  title="Delete"
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/70 p-4 sm:p-8" onClick={() => setSelected(null)}>
          <div className="w-full max-w-2xl rounded-2xl border border-zinc-800 bg-zinc-950 p-6 my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="text-xl font-bold">{selected.invoiceNumber}</h2>
                  <span className={`text-[10px] px-2 py-0.5 rounded border ${STATUS_COLORS[selected.status]}`}>{selected.status}</span>
                </div>
                <div className="text-xs text-zinc-500 mt-1">{selected.clientName || 'No client'}{selected.clientEmail ? ` · ${selected.clientEmail}` : ''}</div>
              </div>
              <button onClick={() => setSelected(null)} className="text-zinc-600 hover:text-white text-xl leading-none">×</button>
            </div>

            {/* Items */}
            <div className="rounded-xl border border-zinc-800 overflow-hidden mb-4">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-zinc-800 text-[10px] uppercase tracking-widest text-zinc-600">
                ...
                <tbody>
                  {(selected.items || []).map((it, i) => (
                    <tr key={i} className="border-b border-zinc-900 last:border-0">
                      ...
                      <td className="px-4 py-2 text-right text-zinc-400">{cur(selected.currency)}{(it.unitPrice || 0).toFixed(2)}</td>
                      <td className="px-4 py-2 text-right font-bold">{cur(selected.currency)}{(it.amount || 0).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {/* Totals */}
            <div className="border-t border-zinc-800 pt-4 mb-4 text-sm">
              <div className="flex justify-between mb-1"><span className="text-zinc-500">Subtotal</span><span>{cur(selected.currency)}{(selected.subtotal || 0).toFixed(2)}</span></div>
              <div className="flex justify-between mb-1"><span className="text-zinc-500">Tax ({selected.taxRate || 0}%)</span><span>{cur(selected.currency)}{(selected.taxAmount || 0).toFixed(2)}</span></div>
              <div className="flex justify-between text-lg font-bold mt-2 pt-2 border-t border-zinc-800"><span>Total</span><span className="text-green-400">{cur(selected.currency)}{(selected.total || 0).toFixed(2)}</span></div>
              <div className="text-[10px] text-zinc-600 mt-2">Due {selected.dueDate}</div>
              {selected.notes && <div className="mt-3 p-3 rounded-lg bg-zinc-900 text-xs text-zinc-400"><span className="uppercase tracking-widest text-zinc-600 text-[10px] block mb-1">Notes</span>{selected.notes}</div>}
            </div>

            {/* Actions */}
            <div className="flex flex-wrap gap-2">
              <button onClick={() => printInvoice(selected)} className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors">
                <FileText className="h-4 w-4" /> Print / Download
              </button>
              {selected.status === 'draft' && (
                <button onClick={() => postInvoiceAction('send', selected)} disabled={actionBusy || !selected.clientEmail} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white rounded-lg px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50" title={!selected.clientEmail ? 'Add a client email to send' : 'Finalize & send this draft'}>
                  {actionBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Send Invoice
                </button>
              )}
              {selected.status === 'sent' && (
                <button onClick={() => postInvoiceAction('markPaid', selected)} disabled={actionBusy} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white rounded-lg px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50">
                  {actionBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Mark Paid
                </button>
              )}
              <button onClick={() => { if (confirm(`Delete invoice ${selected.invoiceNumber}?`)) postInvoiceAction('delete', selected) }} disabled={actionBusy} className="flex items-center gap-2 border border-zinc-800 text-zinc-400 hover:text-red-400 hover:border-red-900 rounded-lg px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50 ml-auto">
