      ...
      try {
        ...
        if (data.cpu >= thresholds.cpu * 1.2) {
          newAlerts.push({
            ...
            message: `CPU usage at ${data.cpu.toFixed(1)}% significantly exceeds ${
              thresholds.cpu
            }% threshold`,
        ...
        if (data.memory >= thresholds.memory * 1.2) {
          newAlerts.push({
            ...
            message: `Memory usage at ${data.memory.toFixed(1)}% significantly exceeds ${
              thresholds.memory
            }% threshold`,
            timestamp: new Date().toISOString(),
            acknowledged: false,
          ...
          newAlerts.push({
            ...
            message: `Memory usage at ${data.memory.toFixed(1)}% exceeds ${
              thresholds.memory
            }% threshold`,
            timestamp: new Date().toISOString(),
            acknowledged: false,
        ...
        if (data.errorRate >= thresholds.errorRate) {
          newAlerts.push({
            ...
            message: `Error rate at ${data.errorRate.toFixed(1)}% exceeds ${
        ...
        if (data.responseTime >= thresholds.responseTime) {
          newAlerts.push({
            ...
            message: `Response time at ${data.responseTime.toFixed(0)}ms exceeds ${
        ...
        <>
          ...
          {currentMetrics && (
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div
                className={`rounded-lg p-3 border ${
                  getThresholdStatus(currentMetrics.errorRate, thresholds.errorRate) === 'critical'
                    ? 'bg-orange-500/10 border-orange-500/30'
                    : getThresholdStatus(currentMetrics.errorRate, thresholds.errorRate) ===
                        'warning'
                      ? 'bg-yellow-500/10 border-yellow-500/30'
                      : 'bg-green-500/10 border-green-500/30'
                }`}
              ...
              <div
                className={`rounded-lg p-3 border ${
                  getThresholdStatus(currentMetrics.responseTime, thresholds.responseTime) ===
                  'critical'
                    ? 'bg-orange-500/10 border-orange-500/30'
                    : getThresholdStatus(currentMetrics.responseTime, thresholds.responseTime) ===
                        'warning'
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setThresholds((prev) => ({ ...prev, cpu: Number(e.target.value) }))
                    }
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setThresholds((prev) => ({ ...prev, memory: Number(e.target.value) }))
                    }
                    className="w-32 accent-orange-500"
                  />
