...
export default function MaintenancePage() {
  ...
  const factoryReset = async () => {
    ...
    if (
      !confirm(
        'Factory reset your agent to the configured stable OpenClaw image? This updates the image, rewrites env vars, and restarts the runtime.'
      )
    )
      return;
    setResetting(true);
    ...
    try {
      ...
      if (res.ok) {
        setRestartMsg(
          `Factory reset complete — pinned to ${
            data.image || 'configured image'
          }. Agent restarting with doctor --fix.`
        );
  ...
  const deleteAgent = async () => {
    ...
    if (
      !confirm(
        'Permanently delete your agent? This removes the container and all data. This cannot be undone.'
      )
    )
      return;
    setDeleting(true);
  ...
  const statusVariant: 'active' | 'idle' | 'error' =
    health?.status === 'healthy'
      ? 'active'
      : health?.status === 'starting' || health?.status === 'stopped' || health?.status === 'setup'
        ? 'idle'
        : 'error';

  return (
    <DashboardShell>
      <DashboardHeader title="Maintenance" icon={<Wrench className="h-5 w-5 text-zinc-500" />} />
      ...
      <DashboardContent className="space-y-px max-w-2xl">
        ...
        <div className="border border-zinc-800 bg-zinc-950 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              {health?.instanceId && (
                <p className="text-[10px] text-zinc-500 font-mono">{health.instanceId}</p>
              )}
            </div>
          ...
          {loading ? (
            ...
            <p className="text-xs text-zinc-500">
              No agent deployed yet. Deploy from the dashboard to get started.
            </p>
          ) : (
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              {health?.statusReason && (
                <div className="mb-6 border border-zinc-800 bg-black p-4">
                  <p className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2">Reason</p>
                  <p className="text-xs text-zinc-400">{health.statusReason}</p>
                </div>
              )}
              {health?.checks?.length ? (
                <div className="mb-6 grid gap-px bg-zinc-900 sm:grid-cols-3">
                  {health.checks.map((check) => (
                    <div key={check.path} className="bg-black p-4">
                      ...
                      {check.reason ? (
                        <p className="mt-1 text-[10px] text-zinc-500">{check.reason}</p>
                      ) : null}
                    </div>
                  ))}
                </div>
            ...
            <div className="pt-4 border-t border-zinc-900">
              <div className="flex flex-wrap gap-3">
                <button
                  ...
                  className="bg-white text-black px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest hover:bg-zinc-200 disabled:bg-zinc-800 disabled:text-zinc-500 transition-colors flex items-center gap-2"
                >
                  {restarting ? (
                    <>
              ...
              <p className="text-[10px] text-zinc-500 mt-3">
                <strong>Run Maintenance:</strong> Restarts agent, runs{' '}
                <span className="font-mono">openclaw doctor --fix</span> on startup.
              </p>
              <p className="text-[10px] text-zinc-500 mt-1">
                <strong>Factory Reset:</strong> Pins OpenClaw to the configured stable image,
                reconfigures env vars, and restarts. Use if your agent broke after an update.
              </p>
              {!controlsEnabled && (
                <div className="mt-3 border border-zinc-800 bg-zinc-900/70 p-3 text-[11px] text-zinc-400">
                  Managed maintenance actions are disabled until the Railway control path is fully
                  verified. Health data above remains live.
                </div>
              )}
              <div className="pt-4 mt-4 border-t border-zinc-900">
                <button
                  ...
                  className="border border-red-900 text-red-600 px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest hover:bg-orange-950 disabled:opacity-50 transition-colors flex items-center gap-2"
                >
                  {deleting ? (
                    <>
                ...
                <p className="text-[10px] text-zinc-500 mt-2">
                  <strong>Delete Agent:</strong> Permanently removes the container and all data.
                  Cannot be undone.
                </p>
              </div>
              {restartMsg && (
                <div
                  className={`mt-3 border p-3 text-[11px] ${
        ...
        <div className="border border-zinc-800 bg-zinc-950">
          ...
          {showMaintenanceGuide && (
            <div className="px-6 pb-6 border-t border-zinc-900 pt-5 space-y-4">
              <p className="text-[11px] text-zinc-400 leading-relaxed">
                If the managed runtime is reachable but stuck in <strong>Stopped</strong> or{' '}
                <strong>Setup Required</strong>, use <strong>Run Maintenance</strong> first. It
                restarts the wrapper and runs the doctor path on startup.
              </p>
              <p className="text-[11px] text-zinc-500">
                If issues persist, use these commands inside the container:
              </p>
              ...
              <div className="border border-zinc-800 bg-zinc-900 p-4">
                ...
                <div className="space-y-2 text-[11px] text-zinc-500 leading-relaxed">
                  <p>
                    <strong className="text-zinc-300">Healthy</strong> means the wrapper is
                    reachable and the runtime is running.
                  </p>
                  <p>
                    <strong className="text-zinc-300">Stopped</strong> means the wrapper is
                    reachable but the process is not running.
                  </p>
                  <p>
                    <strong className="text-zinc-300">Setup Required</strong> means the wrapper is
                    reachable but no final OpenClaw config has been written yet.
                  </p>
                  <p>
                    <strong className="text-zinc-300">Unreachable</strong> should only mean the
                    wrapper itself is not answering its status endpoint.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="border border-zinc-800 bg-zinc-950">
          ...
          {showDocker && (
            <div className="px-6 pb-6 border-t border-zinc-900 pt-5 space-y-3">
              <p className="text-[11px] text-zinc-400 leading-relaxed">
                This wrapper uses <code className="font-mono text-zinc-300">/api/status</code> as
                the authoritative health check. Legacy{' '}
                <code className="font-mono text-zinc-300">/healthz</code> and{' '}
                <code className="font-mono text-zinc-300">/readyz</code> may return 404 depending on
                the wrapper build.
              </p>
              <div className="space-y-2">
                {[
                  {
                    path: '/api/status',
                    label: 'Authoritative wrapper status',
                    color: 'text-green-400',
                  },
                  {
                    path: '/healthz',
                    label: 'Legacy liveness probe (optional)',
                    color: 'text-yellow-400',
                  },
                  {
                    path: '/readyz',
                    label: 'Legacy readiness probe (optional)',
                    color: 'text-yellow-400',
                  },
                ].map(({ path, label, color }) => (
                  <div
                    key={path}
                    className="border border-zinc-800 bg-black p-3 flex items-center justify-between gap-3"
                  >
                    <div>
              ...
              <p className="text-[11px] text-zinc-500">
                If <code className="font-mono text-zinc-300">/api/status</code> returns 200 but the
                dashboard still says unreachable, that is a dashboard interpretation bug, not a
                runtime outage.
              </p>
            </div>
          )}
