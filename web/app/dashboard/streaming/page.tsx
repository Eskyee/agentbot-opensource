...
const platforms: StreamingPlatform[] = [
  {
    ...
    connected: false,
    stats: { listeners: 0, streams: 0, revenue: '$0.00', topTrack: '-' },
  },
  {
    id: 'soundcloud',
    name: 'SoundCloud',
  ...
  },
  {
    id: 'bandcamp',
    name: 'Bandcamp',
    icon: <Disc3 className="h-5 w-5 text-orange-500" />,
    connected: false,
    stats: { listeners: 0, streams: 0, revenue: '$0.00', topTrack: '-' },
  },
  {
  ...
  },
  {
    id: 'youtube-music',
    name: 'YouTube Music',
    icon: <Video className="h-5 w-5 text-zinc-300" />,
    connected: false,
    stats: { listeners: 0, streams: 0, revenue: '$0.00', topTrack: '-' },
  },
];
...
export default function StreamingPage() {
  ...
  const totalListeners = connectedPlatforms
    .filter((p) => p.connected)
    .reduce((acc, p) => acc + p.stats.listeners, 0);
  const totalStreams = connectedPlatforms
    .filter((p) => p.connected)
    .reduce((acc, p) => acc + p.stats.streams, 0);
  ...
  return (
    <DashboardShell>
      ...
      <DashboardContent>
        ...
        <div className="grid gap-px bg-zinc-900 grid-cols-1 sm:grid-cols-3 mb-8">
          <div className="bg-zinc-950 border border-zinc-800 p-5">
            ...
            <div className="text-2xl font-bold tracking-tight">
              {connectedPlatforms.filter((p) => p.connected).length}/{connectedPlatforms.length}
            </div>
          </div>
          <div className="bg-zinc-950 border border-zinc-800 p-5">
            <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2">
        ...
        {/* Platform Cards */}
        <div className="grid gap-px bg-zinc-900 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div key={platform.id} className="bg-zinc-950 border border-zinc-800 p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <StatusPill
                      status={platform.connected ? 'active' : 'offline'}
                ...
                <div className="grid grid-cols-2 gap-px bg-zinc-900 mb-4">
                  <div className="bg-zinc-950 p-3">
                    ...
                    <div className="text-sm font-mono mt-1">
                      {platform.stats.listeners.toLocaleString()}
                    </div>
                  </div>
                  <div className="bg-zinc-950 p-3">
                    <div className="text-[10px] uppercase tracking-widest text-zinc-500">
                    ...
                    <div className="text-sm font-mono mt-1">
                      {platform.stats.streams.toLocaleString()}
                    </div>
                  </div>
                  <div className="bg-zinc-950 p-3">
                    <div className="text-[10px] uppercase tracking-widest text-zinc-500">
              ...
              <button
                ...
                className={`w-full py-3 text-[10px] font-bold uppercase tracking-widest transition-colors ${
                  platform.connected
                    ? 'border border-orange-500/30 text-red-400 hover:border-orange-500'
        ...
        <div className="mt-8 border border-zinc-800 bg-zinc-950 p-6">
          <p className="text-xs text-zinc-500">
            Streaming integrations require OAuth authentication with each platform. Your agent will
            automatically track plays, listeners, and royalties once connected.
          </p>
        </div>
      </DashboardContent>
    </DashboardShell>
