      ...
      <DashboardHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill
              status={
                runtimeState === 'running' || runtimeState === 'active'
                  ? 'active'
              ...
              size="sm"
            />
            <div className="flex items-center gap-px bg-zinc-900">
              {['7d', '30d'].map((period) => (
                <button
                  key={period}
      ...
      <DashboardContent className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-900">
          ...
          <StatCard
            ...
            sub={cronLabel}
            color={gateway?.cron.available ? 'text-orange-400' : 'text-zinc-400'}
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-900">
          <StatCard
            ...
            color={
              servicesHealthy === servicesTotal && servicesTotal > 0
                ? 'text-emerald-400'
                : 'text-amber-300'
            }
          />
          ...
          <StatCard
            ...
            sub={
              costSummary
                ? `$${costSummary.avgCostPerCall.toFixed(4)} avg / call`
                : 'No token usage yet'
            }
            color="text-orange-400"
          />
                ...
                <tbody>
                  {agents.map((agent) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-right text-zinc-500">{agent.model}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="space-y-3">
              {modelBreakdown.map((model) => (
                <div key={model.model} className="flex items-center gap-3">
                  <span className="text-xs font-mono text-zinc-500 w-24 sm:w-40 truncate">
                    {model.model}
                  </span>
