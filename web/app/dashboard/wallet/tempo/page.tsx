...
interface WalletData {
  ...
  chainId: number;
  testnet: boolean;
  totalUsd?: string; // not always returned by /api/wallet; we fall back to primaryToken.balance
...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      startTransition(() => {
        ...
        const active = sessionData?.sessions?.length
          ? sessionData.sessions.find((s: Session) => s.status === 'active')
          : null;
        if (active) {
          setMppSession(active);
          setSessionId(active.id);
        }
        // NOTE: intentionally do NOT call clearSessionId() when no active
        // session is found here. `tempo_session_id` is a shared localStorage
        // key with the Base wallet page; clearing it on Tempo init could wipe
        // a session that Base already opened (and vice versa). The
        // handleDisconnect path is the single authoritative clear.
        setLoading(false);
      });
    });
  }, []);
  ...
  const handleDisconnect = useCallback(() => {
    ...
    setWallet(null);
    setAddressInput('');
    // Also clear any in-memory and persisted MPP session state so a reconnect
    // with a different Tempo address can't reuse the old session id.
    setMppSession(null);
    clearSessionId();
  }, []);
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            // Always reset loading so a falsy data.url (expired Stripe
                            // session) doesn't leave the top-up UI permanently locked.
