      ...
      <DashboardContent>
        <div className="mb-8 border border-zinc-800 bg-zinc-950 p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-[10px] uppercase tracking-widest text-zinc-500">Discovery</div>
              <p className="mt-2 text-sm text-zinc-400">
                Agentbot paid tools use a shared `/api/tools/:tool` surface, exact quotes before
                payment, `Payment-Receipt` on success, and idempotent replay behavior.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link
                href="/openapi.json"
                className="text-xs uppercase tracking-widest text-orange-400 hover:text-white"
              >
                OpenAPI →
              </Link>
              <Link
        ...
        <div className="grid gap-px bg-zinc-900 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="bg-zinc-950 border border-zinc-800 p-5">
            <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-4">Catalog</div>
                ...
                <div key={tool.id} className="border border-zinc-800 bg-black p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {tool.networks.map((network) => (
                      <span
                        key={network}
                        className="border border-zinc-800 px-2 py-1 text-[10px] uppercase tracking-widest text-zinc-500"
                      >
                        {network}
                      </span>
                    ))}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tool.workflowChain.map((chain) => (
                        <span
                          key={chain}
                          className="border border-orange-500/20 bg-orange-500/5 px-2 py-1 text-[10px] uppercase tracking-widest text-orange-400"
                        >
                          {chain}
                        </span>
                      ))}
            ...
            <div className="space-y-4 text-sm text-zinc-400">
              <div className="border border-zinc-800 bg-black p-4">
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  Start broad, then turn the highest-signal themes into social operations.
                </p>
              </div>
              <div className="border border-zinc-800 bg-black p-4">
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2">
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  Find demand, then enrich it with GitHub and GitLawb technical context.
                </p>
              </div>
              <div className="border border-zinc-800 bg-black p-4">
                <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2">
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  A successful call now returns a `Payment-Receipt`, and retries can reuse the same
                  `Idempotency-Key`.
                </p>
              </div>
            </div>
