...
const TEMPLATES = {
  ...
  'fetch-test': { lang: 'javascript', code: 'const res = await fetch("https://api.github.com/zen");\nconsole.log(await res.text());' },
  'math-test': { lang: 'python', code: 'import math\nprint(f"Pi: {math.pi:.6f}")\nprint(f"E: {math.e:.6f}")\nprint(f"Tau: {math.tau:.6f}")' },
}
...
export default function SandboxPage() {
  ...
  const [result, setResult] = useState<{ stdout: string; stderr: string; exitCode: number; duration?: string } | null>(null)
      ...
      <DashboardHeader
        ...
        action={
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-600 font-mono">Firecracker microVM</span>
            <span className="text-[10px] text-emerald-400 bg-emerald-900/20 border border-emerald-800 rounded px-2 py-0.5 font-mono">ISOLATED</span>
          </div>
        }
      />
      ...
      <DashboardContent className="max-w-5xl space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-2">
            {Object.entries(TEMPLATES).map(([key, tmpl]) => (
              <button
                key={key}
                onClick={() => { setCode(tmpl.code); setLanguage(tmpl.lang); setResult(null) }}
        ...
        <div className="border border-zinc-800 bg-zinc-950">
          <div className="border-b border-zinc-800 px-4 py-2 flex items-center justify-between">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Code Editor</span>
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-zinc-600 font-mono">{language === 'python' ? 'Python 3.13' : 'Node.js 24'}</span>
            </div>
          </div>
          <textarea
            value={code}
          ...
          <div className="border border-zinc-800 bg-zinc-950">
            <div className="border-b border-zinc-800 px-4 py-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {result.duration && (
                  <span className="text-[10px] text-zinc-600 font-mono">{result.duration}</span>
                )}
              </div>
              <button
                onClick={copyOutput}
            ...
            <pre className="p-4 text-sm font-mono whitespace-pre-wrap overflow-x-auto min-h-[100px]">
              ...
              {!result.stdout && !result.stderr && <div className="text-zinc-600">No output</div>}
            </pre>
          </div>
        )}
      </DashboardContent>
