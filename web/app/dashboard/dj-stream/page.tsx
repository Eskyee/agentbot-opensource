...
type TabId = 'stream' | 'distribution' | 'relays' | 'guide';
...
function CopyButton({
  ...
  return (
    <button
      ...
      disabled={!value}
      className="inline-flex items-center gap-2 border border-zinc-700 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-zinc-300 transition-colors hover:border-zinc-500 hover:text-white disabled:border-zinc-800 disabled:text-zinc-500"
    >
...
function InfoPanel({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="border border-zinc-800 bg-black p-4">
      <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-3">{title}</div>
      {children}
    </div>
  );
}
...
function EncoderModeButton({
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 text-[10px] uppercase tracking-widest text-zinc-500">{meta}</span>
...
export default function DJStreamPage() {
    ...
    const loadActiveStream = async () => {
      ...
      try {
        ...
        if (data.stream) {
          ...
          setRemainingSeconds(
            typeof data.session?.remaining === 'number' ? data.session.remaining : null
          );
        }
      } catch {
        ...
        try {
          await fetch('/api/relay/destination-key', {
            ...
            body: JSON.stringify({
              ...
              streamKey: youtubeStreamKey.trim(),
            }),
          });
        } catch {
          // Relay server may not be running — key saved to DB for when it comes up
  ...
  const broadcastStatus =
    muxStatus?.streamHealth === 'good'
      ? {
          pill: 'active' as const,
          label: 'Broadcast Live',
          helper: 'Mux is receiving the feed and the station should be ready.',
        }
      : muxStatus?.streamHealth === 'waiting'
        ? {
            pill: 'idle' as const,
            label: 'Waiting for Encoder',
            helper: muxStatus.message || 'Start OBS or ffmpeg to send your program feed.',
          }
        : muxStatus?.streamHealth === 'bad'
          ? {
              pill: 'error' as const,
              label: 'Needs Attention',
              helper: muxStatus.message || 'Mux is not reporting a healthy stream.',
            }
          : {
              pill: 'idle' as const,
              label: 'Stream Armed',
              helper: 'Your stream key is ready. Start OBS or ffmpeg to go live.',
            };
    ...
    <DashboardShell>
      ...
      <DashboardContent>
        <div className="max-w-3xl">
          {streamActionMessage ? (
            <div className="mb-4 border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-xs text-emerald-200">
              {streamActionMessage}
            </div>
          ) : null}
          {error ? (
            <div className="mb-4 border border-orange-500/30 p-3 text-red-400 text-xs">{error}</div>
          ) : null}

          {/* Tab Navigation */}
          <div className="flex gap-px bg-zinc-900 mb-px">
            {TABS.map((tab) => (
              <button
                key={tab.id}
            ...
            <div className="space-y-px">
              ...
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {(isConnected || hasCommunityPass) && (
                    <StatusPill status="active" label={isConnected ? 'Connected' : 'Claimed'} />
                  )}
                </div>
                ...
                {!isConnected ? (
                  <div>
                    ...
                    <p className="mt-2 text-[11px] text-zinc-500">
                      ...
                      <Link href="/settings?tab=wallet" className="text-orange-500 hover:underline">
                        Settings → Wallet
                      </Link>{' '}
                    ...
                    {hasCommunityPass && claimedWallet && (
                      <div className="mt-4 border border-orange-500/20 bg-orange-500/10 p-4">
                        <p className="text-orange-500 text-[10px] uppercase tracking-widest">
                          Agentbot Claimed Wallet
                        </p>
                        <div className="mt-2 flex items-center gap-3">
                          <code className="text-sm text-zinc-200 font-mono">
                            {formatAddress(claimedWallet)}
                          </code>
                          <StatusPill status="active" label="Ready" />
                        </div>
                        <p className="mt-2 text-sm text-zinc-300">
                          Your Agentbot community pass is already linked, so you can stream with the
                          claimed wallet even before connecting a Base wallet.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    claimedWallet.toLowerCase() !== address!.toLowerCase() ? (
                      <div className="border border-orange-500/20 bg-orange-500/10 p-4">
                        ...
                        <p className="mt-2 text-sm text-zinc-300">
                          If you use the Agentbot token pass instead of the RAVE gate, the stream
                          will mint against {formatAddress(claimedWallet)}.
                        </p>
                      </div>
                    ) : null}
                  </div>
                ...
                <div className="border border-zinc-800 bg-zinc-950 p-6">
                  ...
                  <div className="space-y-4">
                    {basefmBalance ? (
                      ...
                      <p className="text-zinc-500 text-xs">Reading on-chain balance...</p>
                    ) : null}

                    {hasCommunityPass && claimedWallet && (
                      <div className="border border-orange-500/20 bg-orange-500/10 p-4">
                        <p className="text-orange-500 text-xs uppercase tracking-widest">
                          Agentbot Community Pass
                        </p>
                        <p className="mt-2 text-sm text-zinc-200">
                          Your {communityProgram?.rewards.currentTier?.label || 'claimed'} holder
                          status unlocks a baseFM guest pass, so you can stream even without the
                          full BASEFM gate.
                        </p>
                        <p className="mt-2 text-xs text-zinc-400">
                          Claimed wallet:{' '}
                          <span className="font-mono text-zinc-200">{claimedWallet}</span>
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="border border-zinc-800 bg-zinc-950 p-6">
                  ...
                  {streamWallet ? (
                    <p className="mt-3 text-xs text-zinc-500">
                      ...
                      {hasCommunityPass && !hasBasefmAccess
                        ? ' · using Agentbot token claim path'
                        : ' · using BASEFM gate path'}
                    </p>
                  ) : null}
                </div>
              )}
              ...
              {stream && (
                <div className="border border-zinc-800 bg-zinc-950 p-6">
                  <SectionHeader
                    ...
                    description="Use this like the station rack after your decks and mixer."
                  />
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <div className="text-left sm:text-right">
                        ...
                        <div className="mt-1 text-[10px] uppercase tracking-widest text-zinc-500">
                          Remaining:{' '}
                          <span className="text-zinc-300">{formatDuration(remainingSeconds)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {relayStreamTarget ? (
                      <div className="space-y-3">
                        <div className="border border-emerald-500/30 bg-emerald-500/5 p-4">
                          <div className="mb-2 flex items-center justify-between gap-3">
                            ...
                            <CopyButton
                              label="Copy Relay RTMP"
                              value={relayStreamTarget}
                              onCopy={copyValue}
                            />
                          </div>
                      ...
                      <div>
                        ...
                        <code className="block bg-black border border-zinc-800 p-3 text-xs text-zinc-400 break-all select-all">
                          {streamTarget ||
                            'Reconnect status loaded. Create a fresh stream if the RTMP key is unavailable.'}
                        </code>
                      </div>
                    )}
                    ...
                    <div className="border border-zinc-800 p-4">
                      ...
                      <div className="mt-4">
                        <div className="mb-2 flex items-center justify-between gap-3">
                          ...
                          <CopyButton
                            label="Copy"
                            value={selectedEncoderCommand}
                            onCopy={copyValue}
                          />
                        </div>
                        <code className="block bg-black border border-zinc-800 p-3 text-xs text-zinc-400 break-all select-all">
                          {selectedEncoderCommand ||
                            'Create a stream to generate the selected encoder command.'}
                        </code>
                      </div>
                    </div>
                    ...
                    <p className="text-xs text-zinc-500">
                      Ending a set normally removes replay files to keep costs down. End Set and
                      Save Replay keeps the finished recording for the owning logged-in account.
                    </p>
                      ...
                      <div className="border border-zinc-800 p-4">
                        ...
                        <div className="grid gap-px bg-zinc-900 sm:grid-cols-3">
                          <div className="bg-black p-3">
                            ...
                            <div className="text-xs text-zinc-300 break-all">
                              {muxStatus.session?.muxStreamId || muxStatus.mux?.id || 'Unknown'}
                            </div>
                          </div>
                          <div className="bg-black p-3">
                            <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2">
                        ...
                        {muxStatus.pickupRecommended ? (
                          <p className="mt-2 text-xs text-amber-300">
                            Your feed is active but the station has not picked it up yet. Refresh
            ...
            <div className="space-y-px">
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                ...
                <div className="mb-6 border border-zinc-800 bg-black p-4">
                  ...
                  <p className="mt-2 text-xs text-zinc-500">
                    One OBS output to the relay server fans out to Mux, X, and YouTube — no
                    per-platform encoder.
                  </p>
                </div>
                                ...
                                {relay.lastHealthyAt ? (
                                  <p>
                                    ...
                                    <span className="text-zinc-300">
                                      {new Date(relay.lastHealthyAt).toLocaleString()}
                                    </span>
                                  </p>
                                ) : null}
                                {relay.lastErrorMessage ? (
                                  <p className="text-red-400">
                                    Last error: {relay.lastErrorMessage}
                                  </p>
                                ) : null}
                              </div>
                            </div>
            ...
            <div className="space-y-px">
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                <SectionHeader
                  ...
                  description="Simulcast your stream to X and YouTube alongside the main baseFM station."
                />
                ...
                <div className="border border-zinc-800 bg-black p-4 mb-4">
                  ...
                  <div className="mb-3 border border-zinc-700/30 bg-zinc-900/50 p-3">
                    <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2">
                      X Studio Stream Settings
                    </div>
                    <div className="space-y-1 text-xs text-zinc-400">
                      <p>
                        RTMP URL: <code className="text-zinc-300">rtmp://ie.pscp.tv:80/x</code>
                      </p>
                      <p>
                        RTMPS URL: <code className="text-zinc-300">rtmps://ie.pscp.tv:443/x</code>
                      </p>
                      <p>
                        Region: <span className="text-zinc-300">EU (Ireland)</span>
                      </p>
                      <p>
                        Recommended:{' '}
                        <span className="text-zinc-300">
                          1080p30, 9Mbps video, 128kbps AAC, keyframe every 3s
                        </span>
                      </p>
                      <p>
                  ...
                  <div className="mb-3 border border-emerald-500/20 bg-emerald-500/10 p-3">
                    <div className="text-[10px] uppercase tracking-widest text-emerald-500 mb-2">
                      Relay Server Simulcasting
                    </div>
                    <div className="space-y-2 text-xs text-zinc-300">
                      <p>
                        Point OBS at the{' '}
                        <strong className="text-zinc-200">Relay Server RTMP</strong> above. The
                        relay forwards your stream to Mux + X + YouTube automatically.
                      </p>
                      <p className="text-zinc-400">
                        Paste your X stream key below. The relay server uses it when pushing to X on
                        your behalf.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <p className="text-xs text-zinc-500">
                      Save to register X as a relay destination and store your stream key for
                      simulcasting.
                    </p>
                    <button
                      onClick={saveXRelay}
                ...
                <div className="border border-zinc-800 bg-black p-4">
                  ...
                  <div className="mb-3 border border-emerald-500/20 bg-emerald-500/10 p-3">
                    ...
                    <div className="space-y-2 text-xs text-zinc-300">
                      <p>
                        Paste your YouTube stream key below. The relay server pushes your stream to
                        YouTube automatically alongside Mux.
                      </p>
                      <p className="text-zinc-500">
                        Get your stream key from YouTube Studio → Go Live → Stream → Stream Key.
                      </p>
                    </div>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <p className="text-xs text-zinc-500">
                      Save to register YouTube as a relay destination and store your stream key for
                      simulcasting.
                    </p>
                    <button
            ...
            <div className="space-y-px">
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                ...
                <div className="mb-4 border border-orange-500/20 bg-orange-500/10 p-4">
                  ...
                  <div className="space-y-2 text-sm text-zinc-200">
                    ...
                    <p>4. Send your mixer master out through OBS to the RTMP target.</p>
                  </div>
                </div>
                ...
                <div className="grid gap-px bg-zinc-900 sm:grid-cols-2 lg:grid-cols-4">
                  <div className="bg-black p-4">
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      Keep track selection, cueing, tempo, EQ, and channel faders on your Pioneer
                      hardware or inside Rekordbox.
                    </p>
                  </div>
                  <div className="bg-black p-4">
                    <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-3">
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      Treat Agentbot like the broadcast rack after your mixer. Your master output
                  ...
                  <div className="bg-black p-4">
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      OBS or your encoder sends the master program feed to baseFM. No need to
                      relearn DJ controls to go live.
                    </p>
                  </div>
                  <div className="bg-black p-4">
                    <div className="text-[10px] uppercase tracking-widest text-zinc-500 mb-3">
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      Agentbot manages first-party playback, basefm.space, and optional downstream
                      relays like YouTube.
                    </p>
                  </div>
                </div>
              ...
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                <InfoPanel title="Test Before Your Set">
                  <div className="space-y-2 text-xs text-zinc-500">
                    <p>
                      Run a short test tone or private audio file first, then press Check Mux
                      Status.
                    </p>
                    <p>
                      For OBS, start streaming for 10-15 seconds and wait for the status to move
                      from Waiting for Encoder to Broadcast Live.
                    </p>
                    <p>
                      For agents, copy the audio-only command and replace{' '}
                      <code className="text-zinc-300">/path/to/set.mp3</code> with a short test file
                      before your real mix.
                    </p>
                  </div>
                </InfoPanel>
              </div>
              ...
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                <InfoPanel title="Recovery">
                  <div className="space-y-2 text-xs text-zinc-500">
                    <p>
                      If the key looks stale or Mux stays idle after your encoder is running, use
                      End Set and create a fresh stream.
                    </p>
                    <p>
                      This browser stores your session token so refreshes keep the controls visible.
                      If you are signed in, Agentbot can also recover the active panel from your
                      account session.
                    </p>
                    <p>
                      On another browser or device, sign in with the same account first; local
                      browser recovery only works where the stream was created.
                    </p>
                  </div>
                </InfoPanel>
              </div>
              ...
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                <div className="border border-zinc-800 p-4">
                  ...
                  <div className="space-y-2 text-xs text-zinc-500">
                    <p>
                      1. Keep cue, beatmatch, loops, hot cues, and EQ work on your Pioneer hardware
                      or Rekordbox.
                    </p>
                    <p>2. Use the mixer master as the source feed for OBS.</p>
                    <p>
                      3. Treat Agentbot like the broadcast rack after the mixer, not a CDJ or mixer
                      replacement.
                    </p>
                    <p>
                      4. If you already know Pioneer workflow, you only need the go-live, stop, and
                      relay controls here.
                    </p>
                  </div>
                </div>
              ...
              <div className="border border-zinc-800 bg-zinc-950 p-6">
                <div className="border border-zinc-800 p-4">
                  <span className="block text-[10px] uppercase tracking-widest text-zinc-500 mb-3">
                    Upload a Mix Set Service
                  </span>
                  <div className="space-y-2 text-xs text-zinc-500">
                    <p>
                      Let DJs upload a finished mix, choose a title and schedule, and have Agentbot
                      broadcast it automatically.
                    </p>
                    <p>Bill separately for the scheduled broadcast and replay retention.</p>
                    <p>
                      Best next step: package it as a managed service for DJs who do not want to run
                      OBS every time.
                    </p>
                  </div>
                  <Link
                    href="/learn/developers/openclaw-dashboard"
                    className="mt-4 inline-flex text-xs uppercase tracking-widest text-orange-500 hover:text-white"
                  >
