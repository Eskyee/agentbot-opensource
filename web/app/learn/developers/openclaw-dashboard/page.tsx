    ...
    <main className="min-h-screen bg-black px-6 py-16 font-mono text-white lg:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-8 border border-zinc-800 bg-zinc-950 p-8">
          ...
          <h1 className="text-4xl font-bold uppercase tracking-tighter">OpenClaw Dashboard and Skill Setup</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-400">
            This page explains how to read Mission Control, what “skills with missing dependencies” actually means,
            and how Agentbot users should move between Runtime, Skills Manager, Config, and broadcasting without getting lost.
          </p>
        </section>

        <section className="mt-8 grid gap-px bg-zinc-800 md:grid-cols-2">
          <div className="bg-black p-6">
            <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600">What The Dashboard Means</p>
            <div className="mt-4 space-y-4 text-sm text-zinc-400">
              <p><span className="font-bold text-white">Runtime</span> tells you whether the managed OpenClaw machine is alive.</p>
              <p><span className="font-bold text-white">Skills Manager</span> tells you what capabilities are installed.</p>
              <p><span className="font-bold text-white">Config</span> is where external channels and keys get finished.</p>
              <p><span className="font-bold text-white">FFmpeg</span> matters for autonomous baseFM DJ output and pre-rendered broadcasts.</p>
            </div>
          </div>
          <div className="bg-black p-6">
            <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600">Missing Dependencies Explained</p>
            <p className="mt-4 text-sm leading-7 text-zinc-400">
              If you see a list like <span className="text-white">1Password, Apple Notes, Apple Reminders +43 more</span>,
              the agent is not broken. Those are optional integrations that need something extra before they can work.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              {dependencyExamples.map((name) => (
                <span key={name} className="rounded-full border border-zinc-800 px-3 py-1 text-[11px] uppercase tracking-[0.14em] text-zinc-300">
                  {name}
                </span>
              ))}
            </div>
        ...
        <section className="mt-8 grid gap-px bg-zinc-800 md:grid-cols-2">
          <div className="bg-black p-6">
            ...
            <p className="mt-4 text-sm leading-7 text-zinc-400">
              Human DJs usually only need three things:
              access, go live, and end set.
              Advanced Mux and relay diagnostics are there for recovery, not as the main workflow.
            </p>
            <Link href="/dashboard/dj-stream" className="mt-4 inline-block text-sm text-orange-500 hover:text-white">
              Open DJ Stream →
            </Link>
          </div>
          <div className="bg-black p-6">
            <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600">Managed Runtime Tip</p>
            <p className="mt-4 text-sm leading-7 text-zinc-400">
            ...
            <Link href="/dashboard" className="mt-4 inline-block text-sm text-orange-500 hover:text-white">
        ...
        <section className="mt-8 border border-zinc-800 bg-zinc-950 p-6">
          <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-600">Paid Broadcast Service</p>
          <h2 className="mt-2 text-2xl font-bold uppercase tracking-tight">Upload a Mix Set and Let Agentbot Broadcast It</h2>
          <p className="mt-4 text-sm leading-7 text-zinc-400">
