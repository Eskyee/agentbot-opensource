 ...
 *
 * MiMo-V2.5-Pro features (from Xiaomi announcement):
 * - Ultra-long context with MTP architecture
 * - 3x higher reasoning efficiency
 * - Full OpenClaw framework compatibility
 * - Kingsoft Office integration (Word, Excel, PPT, PDF)
 *
 * ⚠️ DEPRECATION NOTICE (June 2026):
 * - mimo-v2-flash → mimo-v2.5-pro (auto-route June 18, deprecated June 30)
 * - mimo-v2.5 → mimo-v2.5-pro (auto-route June 18, deprecated June 30)
 * - mimo-v2.5-pro remains the primary model
 */
 ...
 *
 * ⚠️ mimo-v2-flash and mimo-v2.5 are deprecated (auto-route to v2.5-pro June 18, fully removed June 30).
 * Use mimo-v2.5-pro as the default MiMo model.
 */
const LADDER: ModelTier[] = [
  ...
  { id: 'sakana/fugu-ultra', costPer1M: 12, capability: 94 },
...
export function scoreDifficulty(body: Record<string, unknown>): number {
  ...
  const text = messages
    .map((m) => (m && typeof m === 'object' ? (m as { content?: unknown }).content : ''))
    .join('\n');
  ...
  else if (/[{};]\s*$/m.test(text) || /\b(function|class|import|def|const)\b/.test(text))
    score += 10;
...
export function stripRouteHint(body: Record<string, unknown>): {
