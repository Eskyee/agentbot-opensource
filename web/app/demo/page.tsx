...
export default function DemoPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-blue-500/30 font-mono flex flex-col">
      {/* Main Content */}
      <div className="flex-1 flex flex-col max-w-7xl mx-auto px-6 py-8 sm:py-12 w-full">
        {/* Page Title */}
        <div className="mb-8 sm:mb-12">
          <p className="text-[10px] uppercase tracking-widest text-zinc-600 mb-4">Live Demo</p>
        ...
        {showApiKeyInput && (
          <div className="mb-6 sm:mb-8 p-4 sm:p-6 bg-zinc-900 border border-zinc-800 max-w-3xl mx-auto w-full">
            ...
            <input
              ...
              className="w-full bg-black border border-zinc-800 px-4 py-2.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-blue-500 font-mono min-h-[44px]"
            />
        ...
        {mode === 'single' && (
          <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full">
            {/* Model Selector */}
            <div className="mb-4 sm:mb-6 flex items-center justify-between gap-2 border-t border-zinc-900 pt-4">
              <div className="flex items-center gap-3 overflow-x-auto">
                ...
                <select
                  ...
                  className="bg-zinc-900 border border-zinc-800 px-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500 min-h-[44px] font-mono"
                >
              ...
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-zinc-500 px-4">
                  ...
                  <div className="w-full max-w-lg mb-4 border-t border-zinc-900 pt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => setInput('What does the subscription include?')} className="text-xs bg-zinc-800 border border-zinc-700 px-3 py-1.5 hover:bg-zinc-700 hover:text-white text-zinc-400 transition-colors min-h-[32px] font-mono">
                        What&apos;s included?
                      </button>
                      <button onClick={() => setInput('How fast can I deploy?')} className="text-xs bg-zinc-800 border border-zinc-700 px-3 py-1.5 hover:bg-zinc-700 hover:text-white text-zinc-400 transition-colors min-h-[32px] font-mono">
                        Deploy speed?
                      </button>
                      <button onClick={() => setInput('Why SaaS vs self-hosted?')} className="text-xs bg-zinc-800 border border-zinc-700 px-3 py-1.5 hover:bg-zinc-700 hover:text-white text-zinc-400 transition-colors min-h-[32px] font-mono">
                        SaaS vs self-hosted?
                      </button>
                    </div>
                  </div>
                  ...
                  {/* Pricing & Costs */}
                  <div className="w-full max-w-lg border-t border-zinc-900 pt-4">
                    <p className="text-[10px] uppercase tracking-widest text-zinc-600 font-bold mb-3 text-left">Pricing & Costs</p>
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => setInput('Are there any hidden fees?')} className="text-xs bg-zinc-800 border border-zinc-700 px-3 py-1.5 hover:bg-zinc-700 hover:text-white text-zinc-400 transition-colors min-h-[32px] font-mono">
                        Hidden fees?
                      </button>
                      <button onClick={() => setInput('How do AI costs work?')} className="text-xs bg-zinc-800 border border-zinc-700 px-3 py-1.5 hover:bg-zinc-700 hover:text-white text-zinc-400 transition-colors min-h-[32px] font-mono">
                        AI costs explained
                      </button>
                      <button onClick={() => setInput('Can I upgrade or downgrade?')} className="text-xs bg-zinc-800 border border-zinc-700 px-3 py-1.5 hover:bg-zinc-700 hover:text-white text-zinc-400 transition-colors min-h-[32px] font-mono">
                        Change plans?
                      </button>
                    </div>
                  </div>
            ...
            {/* Input */}
            <form onSubmit={handleSubmit} className="flex gap-3 safe-area-bottom">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about deploying Agentbot..."
                className="flex-1 bg-zinc-900 border border-zinc-800 px-4 py-3 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-blue-500 min-h-[48px] font-mono"
                disabled={loading}
              />
              <button
                type="submit"
        ...
        {mode === 'compare' && (
          <div className="flex-1 flex flex-col">
            ...
            <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 border-t border-zinc-900 pt-4">
              <div className="flex items-center gap-3 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 -mx-3 px-3 sm:mx-0 sm:px-0">
                ...
                {compareModels.map((modelId, idx) => (
                  <select
                    ...
                    className="bg-zinc-900 border border-zinc-800 px-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500 min-h-[44px] flex-shrink-0 font-mono"
                  >
                    {DEFAULT_MODELS.map(model => (
                      ...
                      <div className="space-y-3">
                        {(compareMessages[modelId] || []).map((msg) => (
                          <div key={msg.id} className={`px-3 py-2 ${
                            msg.role === 'user' ? 'bg-blue-500/20 text-blue-100 border border-blue-500/30' : 'bg-zinc-800 border border-zinc-700 text-zinc-100'
                          }`}>
                            <div className="text-[10px] uppercase tracking-widest font-bold mb-1 opacity-70">
                        ...
                        {compareLoading && (
                          <div className="flex items-center gap-2 text-zinc-500 text-xs">
                            <div className="flex gap-1">
                              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-zinc-500 rounded-full animate-bounce" />
                              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-zinc-500 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-zinc-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                            </div>
            ...
            {/* Input */}
            <form onSubmit={handleSubmit} className="flex gap-3 mt-4 max-w-2xl mx-auto w-full safe-area-bottom">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
