  ...
  return (
    <main className="min-h-screen bg-black text-white font-mono">
      <section className="relative border-b border-zinc-900 overflow-hidden">
        <div className="absolute top-1/2 right-0 -translate-y-1/2 w-[600px] h-[400px] bg-orange-500/5 rounded-full blur-[150px]" />
        <div className="relative max-w-7xl mx-auto px-5 sm:px-6 py-20 sm:py-32">
          ...
          <h1 className="text-4xl sm:text-6xl md:text-7xl font-bold tracking-tighter uppercase leading-[0.9]">
            Solo<br />
            <span className="text-zinc-700">Founder</span>
          </h1>
          <p className="text-zinc-400 text-sm md:text-base max-w-xl leading-relaxed mt-6">Your personal ops team — email triage, calendar management, web research, and autonomous task execution.</p>
        </div>
      </section>
      <section className="border-b border-zinc-900">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 py-14 sm:py-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-900">
            {[
              { icon: '📧', title: 'Email Triage', desc: 'Sort, prioritize, and draft replies for your inbox. 50 emails/day.' },
              { icon: '📅', title: 'Calendar Management', desc: 'Schedule meetings, send reminders, handle rescheduling autonomously.' },
              { icon: '🔍', title: 'Web Research', desc: 'Research competitors, market trends, potential partners. Auto-summarize.' },
              { icon: '⚡', title: 'Task Execution', desc: 'Execute recurring tasks — reports, data entry, file organization.' },
              { icon: '📝', title: 'Contract Review', desc: 'Analyze PDF contracts, extract key terms, flag risks before signing.' },
              { icon: '💰', title: 'Invoice Management', desc: 'Create and send invoices, track payments, follow up on overdue bills.' },
            ].map((f) => (
