...
async function cleanPackageFile(packageRoot, fileName, destinationPath) {
  ...
  if (packageFileText !== sourceTexts.get(fileName)) {
    throw new Error(
      `Refusing to remove ${relative(
        monorepoRoot,
        destinationPath
      )} because it does not match the root ${fileName}.`
    );
  }
...
for (const packageRoot of packageRoots) {
  ...
  if (packageJson.license !== copiedLicense) {
    throw new Error(
      `Cannot ${clean ? 'remove' : 'copy'} ${copiedLicense} LICENSE for ${
        packageJson.name
      }; package.json declares ${packageJson.license ?? 'no license'}.`
    );
  }
