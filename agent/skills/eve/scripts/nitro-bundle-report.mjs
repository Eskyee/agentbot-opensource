...
function formatSignal(kind, label, bytes, totalBytes) {
  ...
  return `${severity.icon} ${kind} \`${label}\` is ${formatBytes(bytes)} (${formatPercent(
    bytes,
    totalBytes
  )})`;
}
...
function renderAsciiBarChart(title, entries, totalBytes) {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    lines.push(
      `${severity.icon} ${shortenedLabels[index].padEnd(labelWidth)} [${createAsciiBar(
        entry.bytes,
        maxBytes,
        chartWidth
      )}] ${formatBytes(entry.bytes)} ${formatPercent(entry.bytes, totalBytes)}`
    );
  }
  ...
  for (const dependency of dependencies) {
    lines.push(
      `| \`${dependency.name}\` | \`${dependency.range}\` | ${
        dependency.optional ? 'optional peer' : ''
      } |`
    );
  }
...
function formatTopPackageSummary(packages, totalBytes, count) {
  return packages
    .slice(0, count)
    .map(
      (pkg) => `\`${pkg.name}\` ${formatBytes(pkg.bytes)} (${formatPercent(pkg.bytes, totalBytes)})`
    )
    .join(', ');
}
...
function summarizePackageTakeaways(publishedPackage) {
  ...
  return [
    `- Package: tarball ${formatBytes(
      publishedPackage.packedSizeBytes
    )}, install footprint ${formatBytes(publishedPackage.installedSizeBytes)} (${formatBytes(
      publishedPackage.installedDependencyBytes
    )} deps)${topDependencySummary}.`,
  ];
}
...
function summarizeFunctionTakeaways(report) {
  ...
  return [
    `- Runtime: ${report.uniqueFunctionCount} payload${
      report.uniqueFunctionCount === 1 ? '' : 's'
    } totaling ${formatBytes(report.uniqueFunctionBytes)}; largest \`${
      largestFunction.relativePath
    }\` is ${formatBytes(largestFunction.totalBytes)}.`,
  ];
}

function formatByteMetricTransition(metric) {
  return `${formatBytes(metric.baseline)} -> ${formatBytes(metric.current)} (${formatSizeDelta(
    metric.delta
  )})`;
}
...
function summarizeDependencyManifestTakeaway(packageComparison) {
  ...
  if (packageComparison.runtimeDependenciesAdded.length > 0) {
    parts.push(
      `${packageComparison.runtimeDependenciesAdded.length} runtime dep${
        packageComparison.runtimeDependenciesAdded.length === 1 ? '' : 's'
      } added`
    );
  }
  ...
  if (packageComparison.runtimeDependenciesRemoved.length > 0) {
    parts.push(
      `${packageComparison.runtimeDependenciesRemoved.length} runtime dep${
        packageComparison.runtimeDependenciesRemoved.length === 1 ? '' : 's'
      } removed`
    );
  }
  ...
  if (packageComparison.runtimeDependenciesChanged.length > 0) {
    parts.push(
      `${packageComparison.runtimeDependenciesChanged.length} runtime dep${
        packageComparison.runtimeDependenciesChanged.length === 1 ? '' : 's'
      } changed`
    );
  }
  ...
  if (packageComparison.peerDependenciesAdded.length > 0) {
    parts.push(
      `${packageComparison.peerDependenciesAdded.length} peer dep${
        packageComparison.peerDependenciesAdded.length === 1 ? '' : 's'
      } added`
    );
  }
  ...
  if (packageComparison.peerDependenciesRemoved.length > 0) {
    parts.push(
      `${packageComparison.peerDependenciesRemoved.length} peer dep${
        packageComparison.peerDependenciesRemoved.length === 1 ? '' : 's'
      } removed`
    );
  }
  ...
  if (packageComparison.peerDependenciesChanged.length > 0) {
    parts.push(
      `${packageComparison.peerDependenciesChanged.length} peer dep${
        packageComparison.peerDependenciesChanged.length === 1 ? '' : 's'
      } changed`
    );
  }
...
function summarizeComparisonTakeaways(comparison) {
  ...
  if (failedSizeBudgetChecks.length > 0) {
    lines.push(
      `- Bundle warning: ${failedSizeBudgetChecks
        .map((check) => summarizeBundleWarningCheck(check))
        .join('; ')}.`
    );
  }
  ...
  if (notableChangedFunctions.length > 0) {
    runtimeParts.push(
      `${notableChangedFunctions.length} changed payload${
        notableChangedFunctions.length === 1 ? '' : 's'
      }`
    );
  }
...
function renderSummaryTable(report) {
  ...
  if (report.publishedPackage) {
    ...
    lines.push(
      `| Package | Unpacked publish size | ${formatBytes(
        report.publishedPackage.unpackedSizeBytes
      )} |`
    );
    lines.push(
      `| Package | Installed footprint | ${formatBytes(
        report.publishedPackage.installedSizeBytes
      )} |`
    );
  }
...
function renderDependencyManifestDelta(packageComparison, baselineLabel) {
  ...
  if (runtimeChangeCount > 0) {
    ...
    if (packageComparison.runtimeDependenciesAdded.length > 0) {
      lines.push(
        `- Added: ${packageComparison.runtimeDependenciesAdded
    ...
    if (packageComparison.runtimeDependenciesRemoved.length > 0) {
      lines.push(
        `- Removed: ${packageComparison.runtimeDependenciesRemoved
          .map((dependency) => `\`${dependency}\``)
          .join(', ')}`
      );
    }
    ...
    if (packageComparison.runtimeDependenciesChanged.length > 0) {
      lines.push(
        `- Changed: ${packageComparison.runtimeDependenciesChanged
          .map((dependency) => `\`${dependency.baseline}\` -> \`${dependency.current}\``)
          .join(', ')}`
      );
    }
  ...
  if (peerChangeCount > 0) {
    ...
    if (packageComparison.peerDependenciesAdded.length > 0) {
      lines.push(
        `- Added: ${packageComparison.peerDependenciesAdded
          .map((dependency) => `\`${dependency}\``)
          .join(', ')}`
      );
    }
    ...
    if (packageComparison.peerDependenciesRemoved.length > 0) {
      lines.push(
        `- Removed: ${packageComparison.peerDependenciesRemoved
          .map((dependency) => `\`${dependency}\``)
          .join(', ')}`
      );
    }
    ...
    if (packageComparison.peerDependenciesChanged.length > 0) {
      lines.push(
        `- Changed: ${packageComparison.peerDependenciesChanged
          .map((dependency) => `\`${dependency.baseline}\` -> \`${dependency.current}\``)
          .join(', ')}`
      );
    }
...
function renderFunctionDeltaSection(comparison) {
  ...
  const lines = [
    '<details>',
    `<summary>Changed function payloads vs <code>${escapeHtml(comparison.baselineLabel)}</code> (${
      changedFunctions.length
    })</summary>`,
    '',
  ...
  for (const functionComparison of changedFunctions) {
    lines.push(
      `| \`${functionComparison.relativePath}\` | ${functionComparison.status} | ${formatBytes(
        functionComparison.totalBytes.baseline
      )} | ${formatBytes(functionComparison.totalBytes.current)} | ${formatSizeDelta(
        functionComparison.totalBytes.delta
      )} | ${summarizeFunctionRouteDelta(functionComparison)} |`
    );
  }
...
function renderSizeBudgetSection(report) {
  ...
  for (const check of failedChecks) {
    ...
    lines.push(
      `| ${check.area} | ${check.metric} | ${formatBytes(check.baseline)} -> ${formatBytes(
        check.current
      )}; ${formatSizeDelta(check.delta)} (${formatRatioPercent(
        check.increaseRatio
      )}) over limit ${formatRatioPercent(check.thresholdRatio)} |`
    );
  }
...
function renderComparisonSection(comparison) {
  ...
  if (comparison.package !== null) {
    lines.push(
      `| Package | Packed tarball | ${formatBytes(
        comparison.package.packedSizeBytes.baseline
      )} | ${formatBytes(comparison.package.packedSizeBytes.current)} | ${formatSizeDelta(
        comparison.package.packedSizeBytes.delta
      )} |`
    );
    lines.push(
      `| Package | Unpacked publish size | ${formatBytes(
        comparison.package.unpackedSizeBytes.baseline
      )} | ${formatBytes(comparison.package.unpackedSizeBytes.current)} | ${formatSizeDelta(
        comparison.package.unpackedSizeBytes.delta
      )} |`
    );
    lines.push(
      `| Package | Installed footprint | ${formatBytes(
        comparison.package.installedSizeBytes.baseline
      )} | ${formatBytes(comparison.package.installedSizeBytes.current)} | ${formatSizeDelta(
        comparison.package.installedSizeBytes.delta
      )} |`
    );
    lines.push(
      `| Package | Published files | ${comparison.package.publishedFileCount.baseline} | ${
        comparison.package.publishedFileCount.current
      } | ${formatSignedCount(comparison.package.publishedFileCount.delta)} |`
    );
    lines.push(
      `| Package | Installed files | ${comparison.package.installedFileCount.baseline} | ${
        comparison.package.installedFileCount.current
      } | ${formatSignedCount(comparison.package.installedFileCount.delta)} |`
    );
  }

  lines.push(
    `| Runtime | Unique function payloads | ${comparison.app.uniqueFunctionCount.baseline} | ${
      comparison.app.uniqueFunctionCount.current
    } | ${formatSignedCount(comparison.app.uniqueFunctionCount.delta)} |`
  );
  lines.push(
    `| Runtime | Total function bytes | ${formatBytes(
      comparison.app.uniqueFunctionBytes.baseline
    )} | ${formatBytes(comparison.app.uniqueFunctionBytes.current)} | ${formatSizeDelta(
      comparison.app.uniqueFunctionBytes.delta
    )} |`
  );
  lines.push(
    `| Runtime | Public routes | ${comparison.app.publicRouteCount.baseline} | ${
      comparison.app.publicRouteCount.current
    } | ${formatSignedCount(comparison.app.publicRouteCount.delta)} |`
  );
  lines.push('');
...
function renderMetadataSection(report) {
  ...
  lines.push(
    `- Nitro: \`${report.nitroMetadata?.frameworkName ?? 'nitro'}@${
      report.nitroMetadata?.frameworkVersion ?? 'unknown'
    }\``
  );
  lines.push(
...
function renderPublishedPackageSection(publishedPackage) {
  ...
  lines.push(
    `- Package directory: \`${normalizePath(
      relative(process.cwd(), publishedPackage.packageRoot)
    )}\``
  );
  lines.push(
    `- Tarball: ${formatBytes(publishedPackage.packedSizeBytes)}${
      publishedPackage.tarballFilename ? ` (\`${publishedPackage.tarballFilename}\`)` : ''
    }`
  );
  lines.push(
    `- Unpacked payload: ${formatBytes(publishedPackage.unpackedSizeBytes)} across ${
      publishedPackage.publishedFileCount
    } published file${publishedPackage.publishedFileCount === 1 ? '' : 's'}`
  );
  lines.push(
    `- Installed footprint: ${formatBytes(publishedPackage.installedSizeBytes)} across ${
      publishedPackage.installedFileCount
    } installed file${publishedPackage.installedFileCount === 1 ? '' : 's'}`
  );
  ...
  lines.push(
    `- Peer dependencies: ${publishedPackage.peerDependencies.length}${
      publishedPackage.peerDependencies.some((dependency) => dependency.optional)
        ? ` (${
            publishedPackage.peerDependencies.filter((dependency) => dependency.optional).length
          } optional)`
        : ''
    }`
  );
...
function renderFunctionSection(functionEntry) {
  ...
  lines.push(
    `<summary>${functionSeverity.icon} <code>${
      functionEntry.relativePath
    }</code> • ${formatRouteCount(
      functionEntry.publicRoutes.length,
      functionEntry.internalRoutes.length
    )} • ${formatBytes(functionEntry.totalBytes)}</summary>`
  );
  lines.push('');
  ...
  lines.push(
    `| Function files | ${formatBytes(functionEntry.functionFilesBytes)} across ${
      functionEntry.fileCount
    } file${functionEntry.fileCount === 1 ? '' : 's'} |`
  );
