    ...
    for (let j = i; j < Math.min(lines.length, i + 80); j++) {
      ...
      if (depth === 2 && forbiddenKeyRe.test(inner)) {
        ...
        violations.push({
          ...
          line: j + 1,
          message: `authored ${fn}({ ${
            matchedKey ?? forbiddenKeys[0]
          }: ... }) — derive the identifier from the file path instead. Adding an authored \`${
            matchedKey ?? forbiddenKeys[0]
          }\` creates a redundant source of truth that can drift from the path. Remove the field; the framework derives "${
            posix
              .split('/')
              .pop()
              ?.replace(/\.[^.]+$/, '') ?? '<filename>'
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      violations.push({
        rule: 29,
        file: relPath,
        message: `changeset frontmatter must be valid YAML: ${
...
async function main() {
  ...
  for (const { file, was, now } of diffCounts(state.rule13.current, state.rule13.baseline)) {
    violations.push({
      rule: 13,
      file,
      message: `${now} spread-ternary object composition${
        now === 1 ? '' : 's'
      } detected (baseline: ${was}). Replace \`...(cond ? {} : { key: value })\` with explicit assignment: declare the object, then \`if (cond) obj.key = value;\` (or use the conditional form for the *value* not the spread).`,
    });
  }
  ...
  for (const { file, was, now } of diffCounts(state.rule23.current, state.rule23.baseline)) {
    violations.push({
      rule: 23,
      file,
      message: `${now} \`as unknown as T\` cast${
        now === 1 ? '' : 's'
      } detected (baseline: ${was}). Avoid double casts through \`unknown\` — they hide real type errors. Try a direct \`as T\`, fix the source type, or thread a properly typed parameter through. To lower the baseline after a cleanup, regenerate the baseline file. The baseline may shrink, never grow.`,
    });
  }
  ...
  for (const [file, count] of state.rule25.new) {
    violations.push({
      ...
      message: `${count} call${
        count === 1 ? '' : 's'
      } to install/reset/clear runtime-session helpers. Tests must scope runtime state through createTestRuntime().run(fn) / runAsSession(init, fn) / withRuntimeSession(...). Direct calls mutate the process-default RuntimeSession and leak state across tests.`,
    });
  }
  ...
  process.stderr.write(
    `[eve:guard:invariants] FAIL: ${violations.length} violation${
      violations.length === 1 ? '' : 's'
    } of framework mechanical rules.\n\n`
  );
