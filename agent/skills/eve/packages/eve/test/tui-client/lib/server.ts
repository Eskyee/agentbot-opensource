...
async function waitForAgentServer(input: {
    ...
    if (healthResponse?.ok) {
      ...
      if (infoResponse?.ok) {
        ...
        if (agentName !== undefined) {
          throw new Error(
            `Expected smoke target ${JSON.stringify(input.appName)} at ${
              input.baseUrl
            }, but ${JSON.stringify(agentName)} is responding there.`
          );
        }
        throw new Error(
          `Agent server ${JSON.stringify(
            input.appName
          )} returned an unrecognized identity payload from ${input.baseUrl}/eve/v1/info.`
        );
      }
    }
  ...
  throw new Error(
    `Agent server ${JSON.stringify(
      input.appName
    )} did not become healthy with the expected identity within ${HEALTH_TIMEOUT_MS / 1000}s at ${
      input.baseUrl
    }.`
  );
}
