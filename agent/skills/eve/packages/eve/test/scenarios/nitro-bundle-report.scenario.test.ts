...
describe('nitro bundle report', () => {
  ...
  it('renders metric deltas against a main baseline snapshot', async () => {
    ...
    expect(markdown).toContain(
      `| Runtime | Total function bytes | ${formatReportBytes(
        comparison?.app.uniqueFunctionBytes.baseline ?? 0
      )} | ${formatReportBytes(
        comparison?.app.uniqueFunctionBytes.current ?? 0
      )} | ${formatReportSizeDelta(comparison?.app.uniqueFunctionBytes.delta ?? 0)} |`
    );
    expect(markdown).toContain(
      `| \`functions/admin.func\` | added | 0 B | ${formatReportBytes(
        addedFunction?.totalBytes.current ?? 0
      )} | ${formatReportSizeDelta(addedFunction?.totalBytes.delta ?? 0)} |`
    );
    expect(markdown).toContain(
      `| \`functions/__server.func\` | changed | ${formatReportBytes(
        serverComparison?.totalBytes.baseline ?? 0
      )} | ${formatReportBytes(
        serverComparison?.totalBytes.current ?? 0
      )} | ${formatReportSizeDelta(serverComparison?.totalBytes.delta ?? 0)} |`
    );
  }, 15_000);
