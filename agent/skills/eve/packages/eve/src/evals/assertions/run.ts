...
export function calledTool(name: string, options: EveEvalToolCallMatchOptions = {}): RunAssertion {
  return {
    ...
    evaluate(result) {
      ...
      const observed =
        named.length > 0
          ? `observed ${name} calls: ${named
              .map((call) => truncate(JSON.stringify(call.input)))
              .join(', ')}`
...
export function toolOrder(names: readonly string[]): RunAssertion {
  return {
    ...
    evaluate(result) {
      ...
      return fail(
        `missing "${names[cursor]}" after [${names
          .slice(0, cursor)
          .join(', ')}]; observed order: [${observed.join(', ')}]`
      );
    },
  };
}
...
export function outputEquals(value: unknown): RunAssertion {
  return {
    ...
    evaluate(result) {
      ...
      return fail(
        `output ${truncate(JSON.stringify(result.output))} does not equal expected ${truncate(
