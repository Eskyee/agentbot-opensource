...
function renderJUnit(summary: EveEvalRunSummary, input: { readonly suiteName?: string }): string {
  ...
  return [
    ...
    `<testsuite name="${escapeXml(input.suiteName ?? 'eve evals')}" tests="${
      summary.results.length
    }" failures="${failures}" skipped="0" time="${formatSeconds(durationSeconds(summary))}">`,
    ...cases,
    '</testsuite>',
    '',
  ].join('\n');
...
function renderTestCase(result: EveEvalResult): string {
  const attrs = `classname="eve.eval" name="${escapeXml(result.id)}" time="${formatSeconds(
  ...
  return [
    ...
    `    <failure message="${escapeXml(message)}">${escapeXml(
      JSON.stringify(buildFailureDetail(result), null, 2)
    )}</failure>`,
    '  </testcase>',
  ].join('\n');
}
