...
export class ConsoleReporter implements EvalReporter {
  ...
  onRunStart(evaluations: readonly EveEval[], target: EveEvalTarget): void {
    this.#log('');
    this.#log(
      `${this.#colors.bold(this.#colors.cyan('EVALS'))} ${this.#colors.bold(
        String(evaluations.length)
      )}`
    );
    this.#log(
      `${this.#colors.dim('target')} ${
        target.kind === 'local' ? this.#colors.green(target.url) : this.#colors.blue(target.url)
      }`
    );
    this.#log('');
  }
