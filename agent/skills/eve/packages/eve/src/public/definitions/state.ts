...
export function defineState<T>(name: string, initial: () => T): StateHandle<T> {
  if (name.startsWith(RESERVED_STATE_NAME_PREFIX)) {
    throw new Error(
      `defineState() name "${name}" uses the reserved "${RESERVED_STATE_NAME_PREFIX}" prefix, which eve reserves for its own framework context keys (e.g. "eve.channel", "eve.bundle"). Colliding with one silently corrupts context serialization. Use your own namespace, e.g. "my-agent.${name.slice(
        RESERVED_STATE_NAME_PREFIX.length
      )}".`
    );
  }
