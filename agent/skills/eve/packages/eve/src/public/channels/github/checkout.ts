...
export async function checkoutGitHubRepository(
  ...
  await runCheckoutCommand({
    command: `cd ${shellQuote(
      checkoutPath
    )} && GIT_TERMINAL_PROMPT=0 git fetch${fetchDepth} origin ${shellQuote(checkoutRef)}`,
  ...
  if (input.includeBase === true && descriptor.baseSha !== null) {
    await runCheckoutCommand({
      command: `cd ${shellQuote(
        checkoutPath
      )} && GIT_TERMINAL_PROMPT=0 git fetch${fetchDepth} origin ${shellQuote(descriptor.baseSha)}`,
