...
export function vercelSubject(input: VercelSubjectInput): string {
  ...
  if (
    ...
    throw new Error(
      `vercelSubject: invalid environment ${JSON.stringify(
        environment
...
function assertVercelSubjectSegment(field: 'teamSlug' | 'projectName', value: string): void {
  ...
  if (value.includes('*') || value.includes(':')) {
    throw new Error(
      `vercelSubject: ${field} ${JSON.stringify(value)} may not contain ${
        value.includes(':') ? "':'" : "'*'"
      }. Hand-write the subject string when wildcards are intentional.`
    );
  }
}
