...
function resolveDevelopmentServerPort(port: number | string | undefined): number {
  ...
  if (
    ...
    throw new Error(
      `Invalid development server port "${String(
        port
      )}". Expected an integer between 0 and ${MAX_ALLOWED_DEVELOPMENT_SERVER_PORT}.`
    );
  }
...
export async function startDevelopmentServer(
  ...
  try {
    ...
    const { startAuthoredSourceWatcher } = await import(
