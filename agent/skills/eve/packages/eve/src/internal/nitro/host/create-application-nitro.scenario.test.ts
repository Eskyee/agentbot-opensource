...
describe('createApplicationNitro', () => {
  ...
  it('preserves workflow bundle side effects and skips workflow transform for cached bundles', async () => {
    ...
    const { createApplicationNitro } = await import(
      '#internal/nitro/host/create-application-nitro.js'
    );
    ...
    expect(
      sideEffectsPlugin.resolveId?.('./workflows.mjs', '/tmp/.nitro/workflow/workflows-handler.mjs')
    ).toEqual({
