...
function buildWorkflowFileHandlerSource(input: {
  ...
  if (input.directHandlers.length > 0 && input.runtimeImportSpecifier !== undefined) {
    ...
    lines.push(
      `import { getWorld as __eveGetWorkflowWorld } from ${JSON.stringify(
        input.runtimeImportSpecifier
      )};`,
      '',
      'try {',
    ...
    for (const handler of handlerBindings) {
      lines.push(
        `    __eveWorkflowWorld.registerHandler(${JSON.stringify(handler.queuePrefix)}, ${
          handler.binding
        });`
      );
    }
