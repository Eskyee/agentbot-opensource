...
async function waitForHealth(input: {
  ...
  while (Date.now() < deadline) {
    ...
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(
        `Built server process exited (code=${String(child.exitCode)}, signal=${String(
          child.signalCode
...
async function waitForReady(input: {
  ...
  while (Date.now() < deadline) {
    ...
    if (input.child.exitCode !== null || input.child.signalCode !== null) {
      throw new Error(
        `Built server process exited (code=${String(input.child.exitCode)}, signal=${String(
  ...
  const wait = new Promise<void>((resolveWait, rejectWait) => {
    ...
    child.once('exit', (code, signal) => {
      ...
      rejectWait(
        new Error(
          [
            `Built server process exited unexpectedly (code=${String(code)}, signal=${String(
              signal
            )}).`,
            output,
          ].join('\n')
