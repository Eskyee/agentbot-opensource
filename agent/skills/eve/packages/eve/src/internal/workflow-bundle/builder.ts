...
export class WorkflowBundleBuilder {
  ...
  async #performBuild(options: {
    ...
    if (inputFiles.length === 0) {
      throw new Error(
        `Expected the execution workflow source file under "${resolvePackageSourceDirectoryPath(
          'src/execution'
        )}".`
      );
    }
...
async function addStepRegistrationsImport(
  ...
  const importSource = [
    `import { __steps_registered as __eveWorkflowStepsRegistered } from ${JSON.stringify(
      importSpecifier
    )};`,
  ...
  const nextSource = `${source.slice(0, insertionIndex)}${importSource}${source.slice(
    insertionIndex
  )}`;
...
async function rewriteWorkflowCodeLiteral(filePath: string): Promise<void> {
  ...
  const nextSource = `${source.slice(0, valueStart)}${encodeWorkflowCodeLiteral(
    workflowCode
