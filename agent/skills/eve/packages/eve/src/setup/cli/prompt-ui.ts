...
export function formatPromptSubmission(
  ...
  return `${leadingRail}\n${bulletFor(state, options.colors)}  ${options.colors.dim(
    message
  )}${tail}`;
}
...
export function renderMultiselectPrompt<T extends PromptValue>(input: {
  ...
  switch (input.state) {
    ...
    case 'cancel':
      return `${head}${rail}  ${input.colors.strikethrough(
        input.colors.dim('cancelled')
      )}\n${rail}`;
    case 'error': {
...
export function renderSubmitRow(
  ...
  return `${arrow} ${isCursor ? bold : colors.dim(bold)} ${colors.green(
    UNICODE_ROW_GLYPHS.success
  )}`;
}
...
export function renderSearchableSelect<T extends PromptValue>(input: {
  ...
  const moreFooter =
    input.options.length > window.length
      ? `\n${rail}  ${colors.dim(
          `↑↓ ${input.options.length} options, showing ${start + 1}–${end}`
        )}`
      : '';

  const help = searchableHelpLine(rail, colors, input.multiple);
  const body = `${rail}  ${colors.dim(
    ' '
  )} ${filterInput}\n${rail}  ${optionLines}${submitLine}${moreFooter}${help}`;

  if (input.state === 'error') {
    return `${head.trim()}\n${body}\n${cornerFor(input.state, colors)}  ${colors.red(
      input.error ?? ''
    )}\n`;
  }
