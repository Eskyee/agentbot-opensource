...
function unknownSlugError(rawSlug: string): Error {
  return new Error(
    `Unknown connection "${rawSlug}". Known: ${catalogSlugs().join(
      ', '
    )}, or pass a custom name with a definition.`
  );
}
...
function resolveProtocolHeadless(
  ...
  if (effective.length > 1) {
    throw new Error(
      `Connection "${label}" supports multiple protocols (${effective.join(
        ', '
      )}). Pass --protocol to choose one.`
    );
  }
  return effective[0];
}
