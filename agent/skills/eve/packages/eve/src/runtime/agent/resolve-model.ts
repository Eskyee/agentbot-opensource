...
async function loadSourceBackedRuntimeModelReference(
  ...
  const normalizedDefinition = normalizeAgentDefinition(
    definition,
    `Expected the authored agent config export "${
      reference.source.exportName ?? 'default'
    }" from "${reference.source.logicalPath}" to match the public eve shape.`
  );
  ...
  if (model === undefined) {
    throw new Error(
      `Expected the authored agent config export "${
        reference.source.exportName ?? 'default'
      }" from "${reference.source.logicalPath}" to provide a runtime model.`
    );
  }
