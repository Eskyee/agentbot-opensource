...
export function validateAuthorizationSpec(
  ...
  if (hasStart !== hasComplete) {
    return `The "${fieldName}" field must provide either both "startAuthorization" and "completeAuthorization" (interactive OAuth) or neither (getToken-only). Got only "${
      hasStart ? 'startAuthorization' : 'completeAuthorization'
    }".`;
  }
