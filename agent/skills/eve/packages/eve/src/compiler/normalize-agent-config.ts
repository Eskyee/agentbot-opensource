...
export async function compileAgentConfig(
  ...
  const definition = normalizeAgentDefinition(
    ...
    configModule === undefined
      ...
      : `Expected the agent config export "${
          configModule.exportName ?? 'default'
        }" from "${configModulePath}" to match the public eve shape.`
  );
...
async function normalizeAuthoredModelReference(input: {
  ...
  if (
    ...
    throw new Error(
      `Expected the authored agent config export "${source.exportName ?? 'default'}" from "${
        input.sourcePath ?? source.logicalPath
      }" to provide a valid AI SDK language model.`
    );
  }
...
async function withCompiledRuntimeModelLimits(
  ...
  try {
    ...
    throw new Error(
      `Failed to load AI Gateway model metadata for ${input.purpose} "${
        model.id
