  ...
  async function executeStepBody(
    ...
    const prepareModelCallInput = (extraSystemNote?: string) => {
      const extraSystemEntry: SystemModelMessage[] = extraSystemNote
        ...
        : [];
      const baseSystemEntry: SystemModelMessage[] = session.agent.instructions
        ? [{ role: 'system' as const, content: session.agent.instructions }]
        : [];
      ...
      const instructions =
        rawInstructions !== undefined && marker
          ...
          : rawInstructions ?? session.agent.instructions ?? undefined;
    ...
    const runOneModelCall = async (opts: {
      ...
      const executeModelCall = async (): Promise<HarnessStepResult> => {
        if (emit) {
          ...
          } = await emitStreamContent(emit, emissionState, streamResult.stream);
