...
export async function discoverInstructionsSource(input: {
  ...
  if (hasDirectory) {
    const dirResult = await discoverNamedSourceDirectory<InstructionsDefinition>({
      ...
      invalidDirectoryMessage: `Expected "${join(
        input.rootPath,
        'instructions'
      )}" to be a directory of authored instructions.`,
    ...
    return {
      diagnostics: [
        createDiscoverWarningDiagnostic({
          ...
          message: `The "${fileName}" slot is deprecated. Rename it to "${fileName.replace(
            /^system/,
            'instructions'
          )}" — the runtime still loads the legacy slot for now, but support will be removed in a future release.`,
...
export function createSlotCollisionDiagnostic(
  ...
  return createDiscoverErrorDiagnostic({
    ...
    message: `Found conflicting authored sources for "${slotLogicalPath}": ${formatQuotedFileList(
      fileNames
    )}.`,
...
export function createModuleSlotCollisionDiagnostic(
  ...
  return createDiscoverErrorDiagnostic({
    ...
    message: `Found multiple authored module sources for "${slotLogicalPath}": ${formatQuotedFileList(
      fileNames
    )}.`,
    sourcePath: directoryPath,
  });
