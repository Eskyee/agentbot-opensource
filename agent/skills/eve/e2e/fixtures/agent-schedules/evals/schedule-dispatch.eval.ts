...
export default defineEval({
  ...
  async test(t) {
    ...
    if (heartbeat.length === 0) {
      throw new Error(
        `Expected at least one record-heartbeat result in the dispatched session; saw event types ${formatTypes(
          session.events
        )}.`
      );
    }
    if (!heartbeat.some((output) => output.includes(HEARTBEAT_TOKEN))) {
      throw new Error(
        `record-heartbeat ran but no result carried the token ${HEARTBEAT_TOKEN}: ${JSON.stringify(
          heartbeat
        )}`
      );
    }
